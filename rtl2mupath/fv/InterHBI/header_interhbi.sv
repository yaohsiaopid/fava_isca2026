...
wire i0_instn_begin = (id_stage_i.fetch_entry_valid_i && 
...
ISSUE_ONCE: assume property (@(posedge clk_i) i0_instn_begin |=> 
        always !(id_stage_i.fetch_entry_i.address == pc0));
EVENTUAL_ISSUE: assume property (@(posedge clk_i) first |->
    s_eventually(i0_instn_begin));
EXE_IUV: assume property (@(posedge clk_i) i0_instn_begin |-> fetch_ready_id_if);
...
reg i0_inst_begin_hpn;
always @(posedge clk_i) begin
  if (~rst_ni) begin
    i0_inst_begin_hpn <= '0;
  end else begin
    if (i0_instn_begin) begin
      i0_inst_begin_hpn <= 1'b1;
    end 
  end 
end 
I0_PO_I1: assume property (@(posedge clk_i) !i0_inst_begin_hpn -> !i1_instn_begin);
