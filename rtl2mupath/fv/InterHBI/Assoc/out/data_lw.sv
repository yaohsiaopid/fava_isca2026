...
wire [64-1:0] data0;
data0_const: assume property (@(posedge clk_i) CONST(data0));
// register-read stage
i0_data: assume property (@(posedge clk_i) mem_req_s1 |=> 
ex_stage_i.lsu_i.dcache_req_ports_i_ld.data_rdata == data0);
...
DATA: assert property (@(posedge clk_i) 
(scb_0_s13 | scb_1_s13 | scb_2_s13 | scb_3_s13) |->
  commit_stage_i.wdata_o[0] == {{32{data0[31]}}, data0[31:0]} || commit_stage_i.wdata_o[1] == {{32{data0[31]}}, data0[31:0]});

AMO: assume property (@(posedge clk_i) !amo_valid_commit |-> commit_stage_i.amo_resp_i.ack == 1'b0);

// mem_req_s1 |-> ({dcache_req_ports_ex_cache[2].data_wdata} == data0));
wire [64-1:0] addr0;
addr0_const: assume property (@(posedge clk_i) CONST(addr0));
i0_addr: assume property (@(posedge clk_i) issue_s16 |-> 
 (issue_stage_i.i_issue_read_operands.fu_data_o.operand_a + 
  issue_stage_i.i_issue_read_operands.fu_data_o.imm) == addr0);
 ALIGN: assume property (@(posedge clk_i) addr0[2:0] == '0);
