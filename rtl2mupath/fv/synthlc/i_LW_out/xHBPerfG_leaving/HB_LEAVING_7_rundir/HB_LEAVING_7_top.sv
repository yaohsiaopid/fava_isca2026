...
`ifndef WHB
HB_issue_s16_LEAVING_id_stage_s1: assert property (@(posedge clk_i) 
    ($past(!issue_s16_hpn) && issue_s16 && (id_stage_s1_hpn || id_stage_s1)) |=>
     id_stage_s1);
`else 
WHB_issue_s16_LEAVING_id_stage_s1: assert property (@(posedge clk_i) 
    ($past(!issue_s16_hpn) && issue_s16 && id_stage_s1_hpn) |->
    id_stage_s1);
WHB_CONCUR_issue_s16_LEAVING_id_stage_s1: assert property (@(posedge clk_i) 
    ($past(!issue_s16_hpn) && issue_s16 && id_stage_s1_hpn && id_stage_s1) |=>
    (!(id_stage_s1)));
`endif 
