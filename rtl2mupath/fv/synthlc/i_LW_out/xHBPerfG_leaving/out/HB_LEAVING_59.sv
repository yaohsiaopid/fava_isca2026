...
`ifndef WHB
HB_LEAVING_load_unit_op_s3_load_unit_buff_s1: assert property (@(posedge clk_i) 
    ($past(load_unit_op_s3, 2) && 
        $past(load_unit_op_s3) && (!(load_unit_op_s3))) |-> 
    $past(!(load_unit_buff_s1_hpn || load_unit_buff_s1)));
`else 
WHB_LEAVING_load_unit_op_s3_hb_load_unit_buff_s1: assert property (@(posedge clk_i) 
    ($past(load_unit_op_s3, 2) && 
        $past(load_unit_op_s3) && (!(load_unit_op_s3))) |-> $past(!(load_unit_buff_s1_hpn)));
WHB_CONCUR_LEAVING_load_unit_op_s3_hb_load_unit_buff_s1: assert property (@(posedge clk_i) 
    ($past(load_unit_op_s3, 2) && 
        $past(load_unit_op_s3) && (!(load_unit_op_s3))) |-> $past(!(load_unit_buff_s1_hpn) && load_unit_buff_s1));
`endif 
