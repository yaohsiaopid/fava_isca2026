...
`ifndef WHB
HB_LEAVING_id_stage_s1_load_unit_s1: assert property (@(posedge clk_i) 
    ($past(id_stage_s1, 2) && 
        $past(id_stage_s1) && (!(id_stage_s1))) |-> 
    $past(!(load_unit_s1_hpn || load_unit_s1)));
`else 
WHB_LEAVING_id_stage_s1_hb_load_unit_s1: assert property (@(posedge clk_i) 
    ($past(id_stage_s1, 2) && 
        $past(id_stage_s1) && (!(id_stage_s1))) |-> $past(!(load_unit_s1_hpn)));
WHB_CONCUR_LEAVING_id_stage_s1_hb_load_unit_s1: assert property (@(posedge clk_i) 
    ($past(id_stage_s1, 2) && 
        $past(id_stage_s1) && (!(id_stage_s1))) |-> $past(!(load_unit_s1_hpn) && load_unit_s1));
`endif 
