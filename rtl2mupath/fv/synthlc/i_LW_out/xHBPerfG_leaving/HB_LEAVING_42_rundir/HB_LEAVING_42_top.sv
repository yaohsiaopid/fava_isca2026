...
`ifndef WHB
HB_load_unit_buff_s1_LEAVING_id_stage_s1: assert property (@(posedge clk_i) 
    ($past(!load_unit_buff_s1_hpn) && load_unit_buff_s1 && (id_stage_s1_hpn || id_stage_s1)) |=>
     id_stage_s1);
`else 
WHB_load_unit_buff_s1_LEAVING_id_stage_s1: assert property (@(posedge clk_i) 
    ($past(!load_unit_buff_s1_hpn) && load_unit_buff_s1 && id_stage_s1_hpn) |->
    id_stage_s1);
WHB_CONCUR_load_unit_buff_s1_LEAVING_id_stage_s1: assert property (@(posedge clk_i) 
    ($past(!load_unit_buff_s1_hpn) && load_unit_buff_s1 && id_stage_s1_hpn && id_stage_s1) |=>
    (!(id_stage_s1)));
`endif 
