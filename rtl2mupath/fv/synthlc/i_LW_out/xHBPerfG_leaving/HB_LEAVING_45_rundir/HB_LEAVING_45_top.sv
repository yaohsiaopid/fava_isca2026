...
`ifndef WHB
HB_load_unit_buff_s1_LEAVING_load_unit_op_s3: assert property (@(posedge clk_i) 
    ($past(!load_unit_buff_s1_hpn) && load_unit_buff_s1 && (load_unit_op_s3_hpn || load_unit_op_s3)) |=>
     load_unit_op_s3);
`else 
WHB_load_unit_buff_s1_LEAVING_load_unit_op_s3: assert property (@(posedge clk_i) 
    ($past(!load_unit_buff_s1_hpn) && load_unit_buff_s1 && load_unit_op_s3_hpn) |->
    load_unit_op_s3);
WHB_CONCUR_load_unit_buff_s1_LEAVING_load_unit_op_s3: assert property (@(posedge clk_i) 
    ($past(!load_unit_buff_s1_hpn) && load_unit_buff_s1 && load_unit_op_s3_hpn && load_unit_op_s3) |=>
    (!(load_unit_op_s3)));
`endif 
