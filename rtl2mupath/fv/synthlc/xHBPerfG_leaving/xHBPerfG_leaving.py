...
def iso_expr(pl):
    return " || ".join(iso_group_members_in_set(pl))

def is_iso_skip_edge(e):
    """Skip edge if either endpoint is non-canonical."""
    return (iso_repr(e[0]) != e[0] and iso_repr(e[0]) in cv_perflocs_set) or \
           (iso_repr(e[1]) != e[1] and iso_repr(e[1]) in cv_perflocs_set)

def hpn_reg_iso_pair(s1, s2):
    def _reg(pl):
        condition = iso_expr(pl)
        return (
            "\nreg {pl}_hpn;\n"
    ...
    return _reg(s1) + _reg(s2)

def expand_iso_result(fmt_tuple):
    """Given a result tuple of the form (flag0, pl0, flag1, pl1), expand to all
    iso-group combinations of pl0 and pl1 that are present in cv_perflocs."""
    f0, p0, f1, p1 = fmt_tuple
    expanded = []
    for m0 in iso_group_members_in_set(p0):
        for m1 in iso_group_members_in_set(p1):
            if m0 != m1:
                expanded.append((f0, m0, f1, m1))
    return expanded if expanded else [fmt_tuple]
...
def gen():
    ...
    for idx, e in enumerate(edge):
        ...
        # not_s1/not_s2 use iso disjunction so "leaving" covers any group member
        not_s1 = "(!(%s))" % iso_expr(e[0])
        not_s2 = "(!(%s))" % iso_expr(e[1])

        def mod_f(outf, s):
            if outf is None:
                outf = open(fnm, "w")
                outf.write(h_)
                outf.write(hpn_reg_iso_pair(e[0], e[1]))
                if e[1] in over1cyc_pl:
                    outf.write(assume_s2_over_2cyc.format(s2=iso_expr(e[1])))
            outf.write(s)
            return outf
...
def pp():
    ...
    with open("leaving_hb.txt", "w") as f:
        ...
        for itm in agg_res:
            f.write(",".join(itm) + "\n")
    hb_proven_res_exp    = [x for itm in hb_proven_res    for x in expand_iso_result(itm)]
    aws_concur_res_exp   = [x for itm in aws_concur_res   for x in expand_iso_result(itm)]
    whb_leaving_res_exp  = [x for itm in whb_leaving_res  for x in expand_iso_result(itm)]
    ...
    def dedup(lst):
        ...
        return out
    hb_proven_res_exp   = dedup(hb_proven_res_exp)
    aws_concur_res_exp  = dedup(aws_concur_res_exp)
    whb_leaving_res_exp = dedup(whb_leaving_res_exp)

    with open("leaving_hb_proven.txt", "w") as f:
        for itm in hb_proven_res_exp:
            f.write("%d,%s,%d,%s\n" % itm)

    with open("leaving_concur_proven.txt", "w") as f:
        for itm in aws_concur_res_exp:
            f.write("%d,%s,%d,%s\n" % itm)

    aws_concur_set_exp = set(aws_concur_res_exp)
    with open("leaving_whb_proven.txt", "w") as f:
        for itm in whb_leaving_res_exp:
            tmp_itm = (itm[2], itm[3], itm[0], itm[1])
            if not (itm in aws_concur_set_exp or tmp_itm in aws_concur_set_exp):
                f.write("%d,%s,%d,%s\n" % itm)
