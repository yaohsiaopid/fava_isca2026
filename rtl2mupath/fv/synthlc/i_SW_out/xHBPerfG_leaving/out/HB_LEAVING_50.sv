...
`ifndef WHB
HB_store_unit_s1_LEAVING_id_stage_s1: assert property (@(posedge clk_i) 
    ($past(!store_unit_s1_hpn) && store_unit_s1 && (id_stage_s1_hpn || id_stage_s1)) |=>
     id_stage_s1);
`else 
WHB_store_unit_s1_LEAVING_id_stage_s1: assert property (@(posedge clk_i) 
    ($past(!store_unit_s1_hpn) && store_unit_s1 && id_stage_s1_hpn) |->
    id_stage_s1);
WHB_CONCUR_store_unit_s1_LEAVING_id_stage_s1: assert property (@(posedge clk_i) 
    ($past(!store_unit_s1_hpn) && store_unit_s1 && id_stage_s1_hpn && id_stage_s1) |=>
    (!(id_stage_s1)));
`endif 
