...
`ifndef WHB
HB_scb_0_s13_LEAVING_store_unit_s3: assert property (@(posedge clk_i) 
    ($past(!scb_0_s13_hpn) && scb_0_s13 && (store_unit_s3_hpn || store_unit_s3)) |=>
     store_unit_s3);
`else 
WHB_scb_0_s13_LEAVING_store_unit_s3: assert property (@(posedge clk_i) 
    ($past(!scb_0_s13_hpn) && scb_0_s13 && store_unit_s3_hpn) |->
    store_unit_s3);
WHB_CONCUR_scb_0_s13_LEAVING_store_unit_s3: assert property (@(posedge clk_i) 
    ($past(!scb_0_s13_hpn) && scb_0_s13 && store_unit_s3_hpn && store_unit_s3) |=>
    (!(store_unit_s3)));
`endif 
