...
S2_STAY_AT_LEAST_TWO: assume property (@(posedge clk_i) first |-> s_eventually
    ( !id_stage_s1 [*0:$] ##1 id_stage_s1 ##1 id_stage_s1));

// if precondition in unreachable meaning s2 already left when s1 enters
`ifndef WHB
HB_store_unit_s3_LEAVING_id_stage_s1: assert property (@(posedge clk_i) 
    ($past(!store_unit_s3_hpn) && store_unit_s3 && (id_stage_s1_hpn || id_stage_s1)) |=>
     id_stage_s1);
`else 
WHB_store_unit_s3_LEAVING_id_stage_s1: assert property (@(posedge clk_i) 
    ($past(!store_unit_s3_hpn) && store_unit_s3 && id_stage_s1_hpn) |->
    id_stage_s1);
WHB_CONCUR_store_unit_s3_LEAVING_id_stage_s1: assert property (@(posedge clk_i) 
    ($past(!store_unit_s3_hpn) && store_unit_s3 && id_stage_s1_hpn && id_stage_s1) |=>
    (!(id_stage_s1)));
`endif 

`ifndef WHB
HB_LEAVING_store_unit_s3_LEAVING_id_stage_s1: assert property (@(posedge clk_i) 
    ($past(store_unit_s3, 2) && $past(store_unit_s3) && (!(store_unit_s3))) |-> 
    ((id_stage_s1_hpn && id_stage_s1) || !id_stage_s1_hpn));
`else 
WHB_LEAVING_store_unit_s3_LEAVING_id_stage_s1: assert property (@(posedge clk_i) 
    ($past(store_unit_s3, 2) && $past(store_unit_s3) && (!(store_unit_s3))) |-> 
    $past((id_stage_s1_hpn && id_stage_s1) || !id_stage_s1_hpn));
WHB_CONCUR_LEAVING_store_unit_s3_LEAVING_id_stage_s1: assert property (@(posedge clk_i) 
    ($past(store_unit_s3, 2) && $past(store_unit_s3) && (!(store_unit_s3))) |-> 
