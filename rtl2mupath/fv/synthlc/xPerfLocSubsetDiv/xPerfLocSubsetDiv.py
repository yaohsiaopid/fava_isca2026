...
def iso_group_members_in_set(pl, pl_set):
    grp = _iso_group_of.get(pl)
    if grp is None:
        return [pl]
    return [m for m in grp if m in pl_set]

cv_perflocs_set = set(cv_perflocs)
_emitted_hpn = set()
for itm in cv_perflocs:
    canon = iso_repr(itm)
    if canon in _emitted_hpn:
        continue
    _emitted_hpn.add(canon)
    members = iso_group_members_in_set(canon, cv_perflocs_set)
    ...
    h_ += (
        ...
        "        {pl}_hpn <= 1'b1;\n"
        "end\n"
    ).format(pl=canon, cond=condition)
...
class GenComb:
    ...
    def get_all_combination(self, idx):
        ...
        # e weight
        cur = self.elearr[idx]
        # Non-canonical iso member: never include it — the canonical covers this slot.
        # Still recurse with t=0 so the rest of the set is explored normally.
        is_non_canonical = iso_repr(cur) != cur and iso_repr(cur) in cv_perflocs_set
        for t in range(0, 2): #self.arr[idx]):
            if is_non_canonical and t == 1:
                continue
            if t == 1:
                fail = False
...
def gen():
    ...
    for idx, aSet in enumerate(combgen.res):
        ...
        if len(aSet) == 0:
            continue
        if not any("s13" in s for s in aSet):
            continue
        ...
        for loc in cv_perflocs:
            if loc in aSet:
                s_ += "{s1}_hpn & ".format(s1=iso_repr(loc))
