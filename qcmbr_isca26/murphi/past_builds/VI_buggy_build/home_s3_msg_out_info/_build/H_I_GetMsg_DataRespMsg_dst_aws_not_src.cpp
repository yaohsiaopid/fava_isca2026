/******************************
  Program "past_builds/VI_buggy_build/home_s3_msg_out_info/out/H_I_GetMsg_DataRespMsg_dst_aws_not_src.m" compiled by "Caching Murphi Release 5.4.9.1"
...
#define PROTOCOL_NAME "past_builds/VI_buggy_build/home_s3_msg_out_info/out/H_I_GetMsg_DataRespMsg_dst_aws_not_src"
#define BITS_IN_WORLD 408
#define ALIGN
...
/*** Variable declaration ***/
mu_1_Proc mu_in_msg_src("in_msg_src",24);

/*** Variable declaration ***/
mu_1_Proc mu_out_msg_dst("out_msg_dst",32);

/*** Variable declaration ***/
mu_0_boolean mu_sent("sent",40);

/*** Variable declaration ***/
mu_1_MessageType mu_prevRecProcMsg("prevRecProcMsg",48);

/*** Variable declaration ***/
mu_1_HomeState mu_HomeNode("HomeNode",56);

/*** Variable declaration ***/
mu_1__type_2 mu_Procs("Procs",72);

/*** Variable declaration ***/
mu_1_Value mu_LastWrite("LastWrite",120);

/*** Variable declaration ***/
mu_1_Value mu_MainMemory("MainMemory",128);

/*** Variable declaration ***/
mu_1_Message mu_gBuf("gBuf",136);

/*** Variable declaration ***/
mu_1__type_3 mu_ackBus("ackBus",168);

/*** Variable declaration ***/
mu_1__type_4 mu_msgBuf("msgBuf",200);

/*** Variable declaration ***/
mu_0_boolean mu_msgBufNonEmpty("msgBufNonEmpty",328);

/*** Variable declaration ***/
mu_1__type_5 mu_curRequsts("curRequsts",336);
...
mu_out_msg_dst.undefine();
mu_sent = mu_false;
mu_prevRecProcMsg.undefine();
};
...
bool mu__boolexpr6;
bool mu__boolexpr7;
  if (!(!(mu_cur_node.isundefined()))) mu__boolexpr7 = FALSE ;
  else {
  mu__boolexpr7 = ((mu_cur_node) == (mu_selh)) ; 
}
  if (!(mu__boolexpr7)) mu__boolexpr6 = FALSE ;
  else {
  mu__boolexpr6 = ((mu_gBuf.mu_mtype) == (mu_DataRespMsg)) ; 
}
if ( mu__boolexpr6 )
{
mu_sent = mu_true;
...
bool mu__boolexpr8;
bool mu__boolexpr9;
  if (!(!(mu_cur_node.isundefined()))) mu__boolexpr9 = FALSE ;
  else {
  mu__boolexpr9 = ((mu_cur_node) == (mu_selh)) ; 
}
  if (!(mu__boolexpr9)) mu__boolexpr8 = FALSE ;
  else {
  mu__boolexpr8 = ((mu_gBuf.mu_mtype) == (mu_DataRespMsg)) ; 
}
if ( mu__boolexpr8 )
{
if (mu_gBuf.mu_dst.isundefined())
  mu_out_msg_dst.undefine();
else
  mu_out_msg_dst = mu_gBuf.mu_dst;
}
};
/*** end procedure declaration ***/
...
bool mu__boolexpr11;
  ...
  mu__boolexpr11 = ((mu_cur_node) == (mu_selh)) ; 
}
  if (!(mu__boolexpr11)) mu__boolexpr10 = FALSE ;
  else {
  mu__boolexpr10 = ((mu_msg.mu_mtype) == (mu_DataRespMsg)) ; 
}
if ( mu__boolexpr10 )
{
mu_sent = mu_true;
...
bool mu__boolexpr13;
  ...
  mu__boolexpr13 = ((mu_cur_node) == (mu_selh)) ; 
}
  if (!(mu__boolexpr13)) mu__boolexpr12 = FALSE ;
  else {
  mu__boolexpr12 = ((mu_msg.mu_mtype) == (mu_DataRespMsg)) ; 
}
if ( mu__boolexpr12 )
{
if (mu_msg.mu_dst.isundefined())
  mu_out_msg_dst.undefine();
else
  mu_out_msg_dst = mu_msg.mu_dst;
}
};
/*** end procedure declaration ***/
...
void world_class::clear()
{
  ...
  mu_selh.clear();
  mu_in_msg_src.clear();
  mu_out_msg_dst.clear();
  mu_sent.clear();
  mu_prevRecProcMsg.clear();
  mu_HomeNode.clear();
  mu_Procs.clear();
  mu_LastWrite.clear();
  mu_MainMemory.clear();
...
void world_class::reset()
{
  ...
  mu_selh.reset();
  mu_in_msg_src.reset();
  mu_out_msg_dst.reset();
  mu_sent.reset();
  mu_prevRecProcMsg.reset();
  mu_HomeNode.reset();
  mu_Procs.reset();
  mu_LastWrite.reset();
  mu_MainMemory.reset();
...
void world_class::print()
{
  ...
  mu_selh.print();
  mu_in_msg_src.print();
  mu_out_msg_dst.print();
  mu_sent.print();
  mu_prevRecProcMsg.print();
  mu_HomeNode.print();
  mu_Procs.print();
  mu_LastWrite.print();
  mu_MainMemory.print();
...
void world_class::print_statistic()
{
  ...
  mu_in_msg_src.print_statistic();
  mu_out_msg_dst.print_statistic();
  mu_sent.print_statistic();
  mu_prevRecProcMsg.print_statistic();
...
void world_class::print_diff( state *prevstate )
{
  if ( prevstate != NULL )
  {
    ...
    mu_in_msg_src.print_diff(prevstate);
    mu_out_msg_dst.print_diff(prevstate);
    mu_sent.print_diff(prevstate);
    mu_prevRecProcMsg.print_diff(prevstate);
...
void world_class::to_state(state *newstate)
{
  ...
  mu_in_msg_src.to_state( newstate );
  mu_out_msg_dst.to_state( newstate );
  mu_sent.to_state( newstate );
  mu_prevRecProcMsg.to_state( newstate );
...
for(int mu_n = 1; mu_n <= 6; mu_n++)
  if (( ( mu_n >= 1 ) && ( mu_n <= 3 ) )|| ( ( mu_n >= 6 ) && ( mu_n <= 6 ) )) {
bool mu__boolexpr14;
  if (!(mu_allAck)) mu__boolexpr14 = FALSE ;
  else {
  mu__boolexpr14 = (mu_ackBus[mu_n]) ; 
}
mu_allAck = mu__boolexpr14;
};
};
if ( mu_allAck )
{
...
public:
  ...
  bool Condition(unsigned r)
  {
    ...
    r = r / 4;
bool mu__boolexpr15;
  if (!(!(mu_gBuf.mu_src.isundefined()))) mu__boolexpr15 = FALSE ;
  else {
  mu__boolexpr15 = (!(mu_ackBus[mu_n])) ; 
}
  ...
  void NextRule(unsigned & what_rule)
  {
	...
	if ( ( TRUE  ) ) {
bool mu__boolexpr16;
  if (!(!(mu_gBuf.mu_src.isundefined()))) mu__boolexpr16 = FALSE ;
  else {
  mu__boolexpr16 = (!(mu_ackBus[mu_n])) ; 
}
	      if (mu__boolexpr16) {
		if ( ( TRUE  ) )
		  return;
...
bool mu__boolexpr17;
  ...
  mu__boolexpr17 = ((mu_cur_node) == (mu_selh)) ; 
}
if ( mu__boolexpr17 )
{
if (mu_gBuf.mu_mtype.isundefined())
  ...
  mu_prevRecProcMsg = mu_gBuf.mu_mtype;
if (mu_gBuf.mu_src.isundefined())
  mu_in_msg_src.undefine();
else
  mu_in_msg_src = mu_gBuf.mu_src;
}
...
{
bool mu__boolexpr18;
  if (!(!(mu_cur_node.isundefined()))) mu__boolexpr18 = FALSE ;
  else {
  mu__boolexpr18 = ((mu_cur_node) == (mu_selh)) ; 
}
if ( mu__boolexpr18 )
{
if (mu_gBuf.mu_mtype.isundefined())
...
bool mu__boolexpr19;
bool mu__boolexpr20;
  if (!((mu_p.mu_state) == (mu_P_V))) mu__boolexpr20 = FALSE ;
  else {
...
bool mu__boolexpr21;
bool mu__boolexpr22;
  if (!((mu_p.mu_state) == (mu_P_V))) mu__boolexpr22 = FALSE ;
  else {
...
bool mu__boolexpr25;
bool mu__boolexpr26;
  if (!((mu_p.mu_state) == (mu_P_I))) mu__boolexpr26 = FALSE ;
  else {
  mu__boolexpr26 = (!(mu_msgBufNonEmpty)) ; 
}
  if (!(mu__boolexpr26)) mu__boolexpr25 = FALSE ;
  else {
  mu__boolexpr25 = (mu_gBuf.mu_src.isundefined()) ; 
}
	      if (mu__boolexpr25) {
		if ( ( TRUE  ) )
		  return;
...
bool mu__boolexpr27;
bool mu__boolexpr28;
  if (!((mu_p.mu_state) == (mu_P_I))) mu__boolexpr28 = FALSE ;
  else {
  mu__boolexpr28 = (!(mu_msgBufNonEmpty)) ; 
}
  if (!(mu__boolexpr28)) mu__boolexpr27 = FALSE ;
  else {
  mu__boolexpr27 = (mu_gBuf.mu_src.isundefined()) ; 
}
...
bool mu__boolexpr29;
bool mu__boolexpr30;
  if (!((mu_p.mu_state) == (mu_P_I))) mu__boolexpr30 = FALSE ;
  else {
  mu__boolexpr30 = (!(mu_msgBufNonEmpty)) ; 
}
  if (!(mu__boolexpr30)) mu__boolexpr29 = FALSE ;
  else {
  mu__boolexpr29 = (mu_gBuf.mu_src.isundefined()) ; 
}
	      if (mu__boolexpr29) {
		if ( ( TRUE  ) )
		  return;
...
/********************
  Invariant records
 ********************/
int mu__invariant_31() // Invariant "ASSERT_H_I_accept_req_GetMsg"
{
bool mu__boolexpr32;
bool mu__boolexpr33;
bool mu__boolexpr34;
bool mu__boolexpr35;
bool mu__boolexpr36;
  if (!(!(mu_prevHomeNode.isundefined()))) mu__boolexpr36 = FALSE ;
  else {
  mu__boolexpr36 = ((mu_prevHomeNode) == (mu_H_I)) ; 
}
  if (!(mu__boolexpr36)) mu__boolexpr35 = FALSE ;
  else {
  mu__boolexpr35 = (!(mu_prevRecProcMsg.isundefined())) ; 
}
  if (!(mu__boolexpr35)) mu__boolexpr34 = FALSE ;
  else {
  mu__boolexpr34 = ((mu_prevRecProcMsg) == (mu_GetMsg)) ; 
}
  if (!(mu__boolexpr34)) mu__boolexpr33 = FALSE ;
  else {
  mu__boolexpr33 = (mu_sent) ; 
}
  if (!(mu__boolexpr33)) mu__boolexpr32 = TRUE ;
  else {
  mu__boolexpr32 = (!((mu_out_msg_dst) == (mu_in_msg_src))) ; 
}
return mu__boolexpr32;
};

bool mu__condition_37() // Condition for Rule "ASSERT_H_I_accept_req_GetMsg"
{
...
int mu__invariant_38() // Invariant "swmr"
{
bool mu__quant39; 
mu__quant39 = TRUE;
{
...
bool mu__boolexpr41;
bool mu__boolexpr42;
  if (!((mu_c1) != (mu_c2))) mu__boolexpr42 = FALSE ;
  else {
  mu__boolexpr42 = ((mu_Procs[mu_c1].mu_state) == (mu_P_V)) ; 
}
  if (!(mu__boolexpr42)) mu__boolexpr41 = TRUE ;
  else {
  mu__boolexpr41 = ((mu_Procs[mu_c2].mu_state) != (mu_P_V)) ; 
}
if ( !(mu__boolexpr41) )
...
bool mu__condition_43() // Condition for Rule "swmr"
{
...
bool mu__boolexpr46;
  if (!((mu_Procs[mu_n].mu_state) == (mu_P_V))) mu__boolexpr46 = TRUE ;
  else {
  mu__boolexpr46 = ((mu_Procs[mu_n].mu_val) == (mu_LastWrite)) ; 
}
if ( !(mu__boolexpr46) )
...
bool mu__condition_47() // Condition for Rule "values in valid state match last write"
{
...
int mu__invariant_48() // Invariant "value in memory matches value of last write, when invalid"
{
bool mu__boolexpr49;
  if (!((mu_HomeNode.mu_state) == (mu_H_I))) mu__boolexpr49 = TRUE ;
  else {
  mu__boolexpr49 = ((mu_MainMemory) == (mu_LastWrite)) ; 
}
return mu__boolexpr49;
};

bool mu__condition_50() // Condition for Rule "value in memory matches value of last write, when invalid"
{
...
int mu__invariant_51() // Invariant "Invalid implies empty owner"
{
bool mu__boolexpr52;
  if (!((mu_HomeNode.mu_state) == (mu_H_I))) mu__boolexpr52 = TRUE ;
  else {
bool mu__quant53; 
mu__quant53 = TRUE;
{
...
if ( !(!((mu_Procs[mu_n].mu_state) == (mu_P_V))) )
  ...
  mu__boolexpr52 = (mu__quant53) ; 
}
return mu__boolexpr52;
};

bool mu__condition_54() // Condition for Rule "Invalid implies empty owner"
{
...
{"Invalid implies empty owner", &mu__condition_54, NULL, },
{"value in memory matches value of last write, when invalid", &mu__condition_50, NULL, },
{"values in valid state match last write", &mu__condition_47, NULL, },
{"swmr", &mu__condition_43, NULL, },
{"ASSERT_H_I_accept_req_GetMsg", &mu__condition_37, NULL, },
};
...
out_msg_dst:ScalarsetVariable
cur_node:ScalarsetVariable
in_msg_src:ScalarsetVariable
  ...
  if (args->symmetry_reduction.value)
    {
      if (  args->sym_alg.mode == argsym_alg::Exhaustive_Fast_Canonicalize
          ...
          if (Perm.In(i))
            {
              ...
              mu_prevRecProcMsg.Permute(Perm,i);
              ...
              if (args->multiset_reduction.value)
                mu_LastWrite.MultisetSort();
              mu_out_msg_dst.Permute(Perm,i);
              if (args->multiset_reduction.value)
                mu_out_msg_dst.MultisetSort();
              mu_cur_node.Permute(Perm,i);
              if (args->multiset_reduction.value)
                mu_cur_node.MultisetSort();
              mu_in_msg_src.Permute(Perm,i);
              if (args->multiset_reduction.value)
                mu_in_msg_src.MultisetSort();
        ...
        if (ns != workingstate)
          ...
          mu_prevRecProcMsg.Permute(Perm,0);
          if (args->multiset_reduction.value)
            mu_prevRecProcMsg.MultisetSort();
          ...
          if (args->multiset_reduction.value)
            mu_LastWrite.MultisetSort();
          mu_out_msg_dst.Permute(Perm,0);
          if (args->multiset_reduction.value)
            mu_out_msg_dst.MultisetSort();
          mu_cur_node.Permute(Perm,0);
          if (args->multiset_reduction.value)
            mu_cur_node.MultisetSort();
          mu_in_msg_src.Permute(Perm,0);
          if (args->multiset_reduction.value)
            mu_in_msg_src.MultisetSort();
...
void SymmetryClass::Heuristic_Fast_Canonicalize(state* s)
{
  ...
  mu_out_msg_dst.SimpleCanonicalize(Perm);

  mu_cur_node.SimpleCanonicalize(Perm);

  mu_in_msg_src.SimpleCanonicalize(Perm);
