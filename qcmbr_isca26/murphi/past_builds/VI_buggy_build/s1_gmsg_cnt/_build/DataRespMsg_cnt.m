...
if (!isundefined(cur_node) & cur_node = selc & gBuf.mtype = DataRespMsg) then
  cnt := cnt + 1;
endif;
End;
...
Begin
  ...
  msgBuf[src] := msg;
  msgBufNonEmpty := true;
if (!isundefined(cur_node) & cur_node = selc & msg.mtype = DataRespMsg) then
  cnt := cnt + 1;
endif;
End;
...
invariant "core_sent_DataRespMsg_cnt"
  cnt <= 1;
