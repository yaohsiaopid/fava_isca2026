/******************************
  Program "past_builds/VI_buggy_build/core_s3_req_send/out/P_V_GetMsg_P_I_val.m" compiled by "Caching Murphi Release 5.4.9.1"
...
#define PROTOCOL_NAME "past_builds/VI_buggy_build/core_s3_req_send/out/P_V_GetMsg_P_I_val"
...
/********************
  Invariant records
 ********************/
int mu__invariant_21() // Invariant "ASSERT_P_V_to_P_I"
{
bool mu__boolexpr22;
bool mu__boolexpr23;
bool mu__boolexpr24;
  if (!(!(mu_prevProcs.isundefined()))) mu__boolexpr24 = FALSE ;
  else {
  mu__boolexpr24 = ((mu_prevProcs) == (mu_P_V)) ; 
}
  if (!(mu__boolexpr24)) mu__boolexpr23 = FALSE ;
  else {
  mu__boolexpr23 = ((mu_Procs[mu_selc].mu_state) == (mu_P_I)) ; 
}
  if (!(mu__boolexpr23)) mu__boolexpr22 = TRUE ;
  else {
  mu__boolexpr22 = ((mu_prevStateVal) == (mu_Procs[mu_selc].mu_val)) ; 
}
return mu__boolexpr22;
};

bool mu__condition_25() // Condition for Rule "ASSERT_P_V_to_P_I"
{
...
int mu__invariant_26() // Invariant "swmr"
{
bool mu__quant27; 
mu__quant27 = TRUE;
{
...
bool mu__boolexpr29;
bool mu__boolexpr30;
  if (!((mu_c1) != (mu_c2))) mu__boolexpr30 = FALSE ;
  else {
  mu__boolexpr30 = ((mu_Procs[mu_c1].mu_state) == (mu_P_V)) ; 
}
  if (!(mu__boolexpr30)) mu__boolexpr29 = TRUE ;
  else {
  mu__boolexpr29 = ((mu_Procs[mu_c2].mu_state) != (mu_P_V)) ; 
}
if ( !(mu__boolexpr29) )
...
bool mu__condition_31() // Condition for Rule "swmr"
{
...
int mu__invariant_32() // Invariant "values in valid state match last write"
{
bool mu__quant33; 
mu__quant33 = TRUE;
{
for(int mu_n = 1; mu_n <= 3; mu_n++) {
bool mu__boolexpr34;
  if (!((mu_Procs[mu_n].mu_state) == (mu_P_V))) mu__boolexpr34 = TRUE ;
  else {
  mu__boolexpr34 = ((mu_Procs[mu_n].mu_val) == (mu_LastWrite)) ; 
}
if ( !(mu__boolexpr34) )
...
bool mu__condition_35() // Condition for Rule "values in valid state match last write"
{
...
int mu__invariant_36() // Invariant "value in memory matches value of last write, when invalid"
{
bool mu__boolexpr37;
  if (!((mu_HomeNode.mu_state) == (mu_H_I))) mu__boolexpr37 = TRUE ;
  else {
  mu__boolexpr37 = ((mu_MainMemory) == (mu_LastWrite)) ; 
}
return mu__boolexpr37;
};

bool mu__condition_38() // Condition for Rule "value in memory matches value of last write, when invalid"
{
...
int mu__invariant_39() // Invariant "Invalid implies empty owner"
{
bool mu__boolexpr40;
...
if ( !(!((mu_Procs[mu_n].mu_state) == (mu_P_V))) )
  ...
  mu__boolexpr40 = (mu__quant41) ; 
}
return mu__boolexpr40;
};

bool mu__condition_42() // Condition for Rule "Invalid implies empty owner"
{
...
{"Invalid implies empty owner", &mu__condition_42, NULL, },
{"value in memory matches value of last write, when invalid", &mu__condition_38, NULL, },
{"values in valid state match last write", &mu__condition_35, NULL, },
{"swmr", &mu__condition_31, NULL, },
{"ASSERT_P_V_to_P_I", &mu__condition_25, NULL, },
};
