...
if (!isundefined(cur_node) & cur_node = selh) then
 prevSentMsgSet[gBuf.mtype]:= true; endif;
End;
...
Begin
  ...
  msgBufNonEmpty := true;
if (!isundefined(cur_node) & cur_node = selh) then
 prevSentMsgSet[msg.mtype]:= true; endif;
End;
...
invariant "ASSERT_H_V_accept_req_PutMsg"
  (!isundefined(prevHomeNode) & prevHomeNode = H_V & 
   !isundefined(prevRecProcMsg) & prevRecProcMsg = PutMsg) ->
   ( (prevSentMsgSet[GetMsg] = false & prevSentMsgSet[PutMsg] = false & prevSentMsgSet[DataRespMsg] = false) 
);
