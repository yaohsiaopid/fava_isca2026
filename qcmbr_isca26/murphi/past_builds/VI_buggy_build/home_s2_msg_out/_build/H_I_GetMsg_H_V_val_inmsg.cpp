/******************************
  Program "past_builds/VI_buggy_build/home_s2_msg_out/out/H_I_GetMsg_H_V_val_inmsg.m" compiled by "Caching Murphi Release 5.4.9.1"
...
#define PROTOCOL_NAME "past_builds/VI_buggy_build/home_s2_msg_out/out/H_I_GetMsg_H_V_val_inmsg"
...
/*** Variable declaration ***/
mu_1_Value mu_prevRecProcMsgVal("prevRecProcMsgVal",24);

/*** Variable declaration ***/
mu_1_Value mu_prevHomeNodeVal("prevHomeNodeVal",32);

/*** Variable declaration ***/
mu_1_MessageType mu_prevRecProcMsg("prevRecProcMsg",40);

/*** Variable declaration ***/
mu_1_HomeState mu_HomeNode("HomeNode",48);

/*** Variable declaration ***/
mu_1__type_2 mu_Procs("Procs",64);
...
/*** Variable declaration ***/
mu_1__type_3 mu_ackBus("ackBus",160);

/*** Variable declaration ***/
mu_1__type_4 mu_msgBuf("msgBuf",192);

/*** Variable declaration ***/
mu_0_boolean mu_msgBufNonEmpty("msgBufNonEmpty",320);

/*** Variable declaration ***/
mu_1__type_5 mu_curRequsts("curRequsts",328);
...
if (mu_HomeNode.mu_val.isundefined())
  mu_prevHomeNodeVal.undefine();
else
  mu_prevHomeNodeVal = mu_HomeNode.mu_val;
...
void world_class::clear()
{
  ...
  mu_selh.clear();
  mu_prevRecProcMsgVal.clear();
  mu_prevHomeNodeVal.clear();
...
void world_class::reset()
{
  ...
  mu_selh.reset();
  mu_prevRecProcMsgVal.reset();
  mu_prevHomeNodeVal.reset();
...
void world_class::print()
{
  ...
  mu_selh.print();
  mu_prevRecProcMsgVal.print();
  mu_prevHomeNodeVal.print();
...
void world_class::print_statistic()
{
  ...
  mu_prevRecProcMsgVal.print_statistic();
  mu_prevHomeNodeVal.print_statistic();
...
void world_class::print_diff( state *prevstate )
{
  if ( prevstate != NULL )
  {
    ...
    mu_prevRecProcMsgVal.print_diff(prevstate);
    mu_prevHomeNodeVal.print_diff(prevstate);
...
void world_class::to_state(state *newstate)
{
  ...
  mu_prevRecProcMsgVal.to_state( newstate );
  mu_prevHomeNodeVal.to_state( newstate );
...
if (mu_gBuf.mu_val.isundefined())
  ...
  mu_prevRecProcMsgVal = mu_gBuf.mu_val;
}
...
{
bool mu__boolexpr10;
  if (!(!(mu_cur_node.isundefined()))) mu__boolexpr10 = FALSE ;
  else {
  mu__boolexpr10 = ((mu_cur_node) == (mu_selh)) ; 
}
if ( mu__boolexpr10 )
{
if (mu_gBuf.mu_mtype.isundefined())
...
/********************
  Invariant records
 ********************/
int mu__invariant_23() // Invariant "H_I_accept_req_GetMsg"
{
bool mu__boolexpr24;
bool mu__boolexpr25;
bool mu__boolexpr26;
bool mu__boolexpr27;
bool mu__boolexpr28;
  if (!(!(mu_prevHomeNode.isundefined()))) mu__boolexpr28 = FALSE ;
  else {
  mu__boolexpr28 = ((mu_prevHomeNode) == (mu_H_I)) ; 
}
  if (!(mu__boolexpr28)) mu__boolexpr27 = FALSE ;
  else {
  mu__boolexpr27 = (!(mu_prevRecProcMsg.isundefined())) ; 
}
  if (!(mu__boolexpr27)) mu__boolexpr26 = FALSE ;
  else {
  mu__boolexpr26 = ((mu_prevRecProcMsg) == (mu_GetMsg)) ; 
}
  if (!(mu__boolexpr26)) mu__boolexpr25 = FALSE ;
  else {
  mu__boolexpr25 = ((mu_HomeNode.mu_state) == (mu_H_V)) ; 
}
  if (!(mu__boolexpr25)) mu__boolexpr24 = TRUE ;
  else {
  mu__boolexpr24 = ((mu_HomeNode.mu_val) == (mu_prevRecProcMsgVal)) ; 
}
return mu__boolexpr24;
};

bool mu__condition_29() // Condition for Rule "H_I_accept_req_GetMsg"
{
...
{"H_I_accept_req_GetMsg", &mu__condition_29, NULL, },
};
...
prevRecProcMsgVal:ScalarsetVariable
cur_node:ScalarsetVariable
prevHomeNodeVal:ScalarsetVariable
...
void SymmetryClass::MultisetSort(state* s)
{
        ...
        mu_prevRecProcMsgVal.MultisetSort();
        mu_cur_node.MultisetSort();
        mu_prevHomeNodeVal.MultisetSort();
  ...
  if (args->symmetry_reduction.value)
    {
      if (  args->sym_alg.mode == argsym_alg::Exhaustive_Fast_Canonicalize
          ...
          if (Perm.In(i))
            {
              ...
              if (args->multiset_reduction.value)
                mu_LastWrite.MultisetSort();
              mu_prevRecProcMsgVal.Permute(Perm,i);
              if (args->multiset_reduction.value)
                mu_prevRecProcMsgVal.MultisetSort();
              mu_cur_node.Permute(Perm,i);
              if (args->multiset_reduction.value)
                mu_cur_node.MultisetSort();
              mu_prevHomeNodeVal.Permute(Perm,i);
              if (args->multiset_reduction.value)
                mu_prevHomeNodeVal.MultisetSort();
        ...
        if (ns != workingstate)
          ...
          if (args->multiset_reduction.value)
            mu_LastWrite.MultisetSort();
          mu_prevRecProcMsgVal.Permute(Perm,0);
          if (args->multiset_reduction.value)
            mu_prevRecProcMsgVal.MultisetSort();
          mu_cur_node.Permute(Perm,0);
          if (args->multiset_reduction.value)
            mu_cur_node.MultisetSort();
          mu_prevHomeNodeVal.Permute(Perm,0);
          if (args->multiset_reduction.value)
            mu_prevHomeNodeVal.MultisetSort();
...
void SymmetryClass::Heuristic_Fast_Canonicalize(state* s)
{
  ...
  mu_prevRecProcMsgVal.SimpleCanonicalize(Perm);

  mu_cur_node.SimpleCanonicalize(Perm);

  mu_prevHomeNodeVal.SimpleCanonicalize(Perm);
