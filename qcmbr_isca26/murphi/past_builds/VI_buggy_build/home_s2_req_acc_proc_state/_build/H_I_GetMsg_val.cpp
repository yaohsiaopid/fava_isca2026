/******************************
  Program "past_builds/VI_buggy_build/home_s2_req_acc_proc_state/out/H_I_GetMsg_val.m" compiled by "Caching Murphi Release 5.4.9.1"
...
#define PROTOCOL_NAME "past_builds/VI_buggy_build/home_s2_req_acc_proc_state/out/H_I_GetMsg_val"
#define BITS_IN_WORLD 392
#define ALIGN
...
/*** Variable declaration ***/
mu_1_Value mu_prevHomeNodeVal("prevHomeNodeVal",24);

/*** Variable declaration ***/
mu_1_MessageType mu_prevRecProcMsg("prevRecProcMsg",32);

/*** Variable declaration ***/
mu_1_HomeState mu_HomeNode("HomeNode",40);

/*** Variable declaration ***/
mu_1__type_2 mu_Procs("Procs",56);

/*** Variable declaration ***/
mu_1_Value mu_LastWrite("LastWrite",104);

/*** Variable declaration ***/
mu_1_Value mu_MainMemory("MainMemory",112);

/*** Variable declaration ***/
mu_1_Message mu_gBuf("gBuf",120);

/*** Variable declaration ***/
mu_1__type_3 mu_ackBus("ackBus",152);

/*** Variable declaration ***/
mu_1__type_4 mu_msgBuf("msgBuf",184);

/*** Variable declaration ***/
mu_0_boolean mu_msgBufNonEmpty("msgBufNonEmpty",312);

/*** Variable declaration ***/
mu_1__type_5 mu_curRequsts("curRequsts",320);
...
bool mu__boolexpr26;
bool mu__boolexpr27;
  if (!(!(mu_prevHomeNode.isundefined()))) mu__boolexpr27 = FALSE ;
  else {
  mu__boolexpr27 = ((mu_prevHomeNode) == (mu_H_I)) ; 
}
  if (!(mu__boolexpr27)) mu__boolexpr26 = FALSE ;
  else {
  mu__boolexpr26 = (!(mu_prevRecProcMsg.isundefined())) ; 
}
  if (!(mu__boolexpr26)) mu__boolexpr25 = FALSE ;
  else {
  mu__boolexpr25 = ((mu_prevRecProcMsg) == (mu_GetMsg)) ; 
}
  if (!(mu__boolexpr25)) mu__boolexpr24 = TRUE ;
  else {
  mu__boolexpr24 = ((mu_prevHomeNodeVal) == (mu_HomeNode.mu_val)) ; 
}
return mu__boolexpr24;
};

bool mu__condition_28() // Condition for Rule "H_I_accept_req_GetMsg"
{
...
int mu__invariant_29() // Invariant "swmr"
{
bool mu__quant30; 
mu__quant30 = TRUE;
{
...
bool mu__boolexpr32;
bool mu__boolexpr33;
  if (!((mu_c1) != (mu_c2))) mu__boolexpr33 = FALSE ;
  else {
  mu__boolexpr33 = ((mu_Procs[mu_c1].mu_state) == (mu_P_V)) ; 
}
  if (!(mu__boolexpr33)) mu__boolexpr32 = TRUE ;
  else {
  mu__boolexpr32 = ((mu_Procs[mu_c2].mu_state) != (mu_P_V)) ; 
}
if ( !(mu__boolexpr32) )
...
bool mu__condition_34() // Condition for Rule "swmr"
{
...
int mu__invariant_35() // Invariant "values in valid state match last write"
{
bool mu__quant36; 
mu__quant36 = TRUE;
{
...
bool mu__boolexpr37;
  if (!((mu_Procs[mu_n].mu_state) == (mu_P_V))) mu__boolexpr37 = TRUE ;
  else {
  mu__boolexpr37 = ((mu_Procs[mu_n].mu_val) == (mu_LastWrite)) ; 
}
if ( !(mu__boolexpr37) )
...
bool mu__condition_38() // Condition for Rule "values in valid state match last write"
{
...
int mu__invariant_39() // Invariant "value in memory matches value of last write, when invalid"
{
bool mu__boolexpr40;
  if (!((mu_HomeNode.mu_state) == (mu_H_I))) mu__boolexpr40 = TRUE ;
  else {
  mu__boolexpr40 = ((mu_MainMemory) == (mu_LastWrite)) ; 
}
return mu__boolexpr40;
};

bool mu__condition_41() // Condition for Rule "value in memory matches value of last write, when invalid"
{
...
int mu__invariant_42() // Invariant "Invalid implies empty owner"
{
bool mu__boolexpr43;
...
if ( !(!((mu_Procs[mu_n].mu_state) == (mu_P_V))) )
  ...
  mu__boolexpr43 = (mu__quant44) ; 
}
return mu__boolexpr43;
};

bool mu__condition_45() // Condition for Rule "Invalid implies empty owner"
{
...
{"Invalid implies empty owner", &mu__condition_45, NULL, },
{"value in memory matches value of last write, when invalid", &mu__condition_41, NULL, },
{"values in valid state match last write", &mu__condition_38, NULL, },
{"swmr", &mu__condition_34, NULL, },
{"H_I_accept_req_GetMsg", &mu__condition_28, NULL, },
};
