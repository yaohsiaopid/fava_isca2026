...
var
cur_node: Node;
msg_imm_sent_set: array [MessageType] of boolean;
prevMsg: MessageType;
tracked: boolean;
start: boolean;
cur_idx: 0..2;
tar: array[0..0] of ProcStateEnum;
...
if (start) then
  ...
  switch match
    case 0:
      if (msg_imm_sent_set[GetMsg] = false & msg_imm_sent_set[PutMsg] = true & msg_imm_sent_set[DataRespMsg] = false) then match := match + 1; else match := 2; endif;
  endswitch;
 for tmpm: MessageType do msg_imm_sent_set[tmpm] := false; endfor;
endif;

  if (prevProcs != cur_state) then
    if (cur_idx <= 1) then
      if (cur_idx < 1 & cur_state = tar[cur_idx]) then
        cur_idx := cur_idx + 1;
      else 
        cur_idx := 2;
      endif;
...
prevMsg := gBuf.mtype;
endif;if (start & !isundefined(cur_node) & cur_node = selc) then
  msg_imm_sent_set[gBuf.mtype] := true;
endif;
End;
...
prevMsg := msg.mtype;
  ...
  msg_imm_sent_set[msg.mtype] := true;
endif;
End;
...
undefine cur_node;
for m:MessageType do msg_imm_sent_set[m] := false;
endfor;
tracked := false;
start := false;
cur_idx := 0;
...
invariant "swmr"
  ...
  rule "track_P_V_ci_evict"
    !tracked & !IsUndefined(prevProcs) & (prevProcs = P_V) & 
    !IsUndefined(prevProcReq.tp) & (prevProcReq.tp = ci_evict) ==>
 ...
 msg_imm_sent_set [prevMsg] := true;
endif

    
  endrule;
  
invariant "P_V_ci_evict_rset_0"
(tracked & !start & cur_idx = 1) ->
(match != 1)
