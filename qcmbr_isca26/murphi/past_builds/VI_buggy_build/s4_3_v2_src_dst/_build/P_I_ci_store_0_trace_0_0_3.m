...
var
cur_node: Node;
prevHomeNode: HomeStateEnum;
selh: Home;
msg_rec_set: array [MessageType] of boolean;
msg_sent_set: array [MessageType] of boolean;
prevMsg: MessageType;
tracked: boolean;
start: boolean;
...
var
cur_state:ProcStateEnum;
in_stable_state:boolean;

begin


  cur_state := Procs[selc].state;
  in_stable_state := cur_state = P_I|cur_state = P_V;
  if (start) then
    -- BLOCK_WITHIN_START
    
                  if (!isundefined(prevMsgReceiver)) then 
                    chk := chk & (!isundefined(prevHomeNode) & prevHomeNode != HomeNode.state | prevSent);
                  endif;
...
cur_node := m;
prevHomeNode := HomeNode.state;
undefine prevMsg;
prevProcs := Procs[selc].state;
undefine prevProcReq;
undefine prevMsgReceiver;
prevSent := false;
end;
-- Procedures
----------------------------------------------------------------------
...
Begin
  ...
  if (start & !isundefined(cur_node) & cur_node = selc) then
    msg_sent_set[gBuf.mtype] := true;
  endif;
  if (cur_node = selh) then prevSent := true; endif;
End;
...
ruleset n:Node Do
 ...
 book_keep(n);
  ...
  if (start & !isundefined(cur_node) & cur_node = selc) then
    msg_rec_set[gBuf.mtype] := true;
  endif;
  if (start & gBuf.mtype = DataRespMsg & !isundefined(cur_node) & cur_node = selc) then chk := chk & ismember(gBuf.src, Home); endif;
if (start & gBuf.mtype = GetMsg & gBuf.src = selc &  ismember(cur_node, Home)) then prevMsgReceiver := cur_node; endif;
        HomeReceive(gBuf);
      else
...
for m:MessageType do msg_rec_set[m] := false;
endfor;
for m:MessageType do msg_sent_set[m] := false;
endfor;
tracked := false;
start := false;
undefine prevProcReq;
...
invariant "swmr"
  ...
  rule "track_P_I_ci_store"
    !tracked & !IsUndefined(prevProcs) & (prevProcs = P_I) & 
    !IsUndefined(prevProcReq.tp) & (prevProcReq.tp = ci_store) ==>
    -- no need for book_keep() as this time stamp shouldn't perturb the state of
    -- the machine 
    start := true;
    tracked := true;
    
    if (!isundefined(prevMsg)) then
...
invariant "P_I_ci_store_rset_0"
(tracked & !start & (MultisetCount(i:reached_set, reached_set[i] = P_I) = 0) & 
(MultisetCount(i:reached_set, reached_set[i] = P_IVD) = 1) & 
(MultisetCount(i:reached_set, reached_set[i] = P_V) = 1) & 

(msg_rec_set[GetMsg] = true & msg_rec_set[PutMsg] = false & msg_rec_set[DataRespMsg] = true & msg_sent_set[GetMsg] = true & msg_sent_set[PutMsg] = false & msg_sent_set[DataRespMsg] = false)
) ->
(
!chk);
