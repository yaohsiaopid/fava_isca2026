  ...
  if (start) then
    -- BLOCK_WITHIN_START
                  ...
                  if (!isundefined(prevMsgReceiver)) then 
                    chk := chk & (prevProcs2 != Procs[selc2].state | prevSent);
                  endif;
...
Begin
  ...
  if (cur_node = selc2) then prevSent := true; endif;
End;
...
if (start & gBuf.mtype = GetMsg & gBuf.src = selc &  cur_node = selc2) then prevMsgReceiver := cur_node; endif;
        HomeReceive(gBuf);
      else
