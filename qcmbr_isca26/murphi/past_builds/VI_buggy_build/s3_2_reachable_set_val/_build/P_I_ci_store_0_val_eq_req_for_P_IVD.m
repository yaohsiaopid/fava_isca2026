...
if (Procs[selc].state = P_IVD) then 
  assert (trackReq.vld) "not vld trackReq";
  val_chk := (trackReq.tp = ci_store & Procs[selc].val = trackReq.cl);
endif;
...
(tracked & !start & cur_idx = 2 & 
 ...
 true) ->
(val_chk = true);
-- proven: P_IVD is in the reachable_set and value always match current request data argument
