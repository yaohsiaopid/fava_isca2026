...
if (!isundefined(cur_node) & cur_node = selc & 
    ...
    Procs[selc].state = P_I & gBuf.mtype = DataRespMsg) then
  assert (gBuf.val = prevStateVal) "DIFF";
endif;
End;
...
if (!isundefined(cur_node) & cur_node = selc & 
    !isundefined(prevProcs) & prevProcs = P_V & 
    Procs[selc].state = P_I & msg.mtype = DataRespMsg) then
  assert (msg.val = prevStateVal) "DIFF";
endif;
End;
