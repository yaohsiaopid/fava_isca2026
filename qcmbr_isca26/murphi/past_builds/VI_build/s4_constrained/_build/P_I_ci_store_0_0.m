...
start: boolean;
cur_idx: 0..3;
tar: array[0..1] of ProcStateEnum;
...
if (start) then
  ...
  switch match
    case 0:
      if (msg_imm_sent_set[GetMsg] = false & msg_imm_sent_set[PutMsg] = false & msg_imm_sent_set[DataRespMsg] = false) then match := match + 1; else match := 3; endif;
    case 1:
      if (msg_imm_sent_set[GetMsg] = true & msg_imm_sent_set[PutMsg] = false & msg_imm_sent_set[DataRespMsg] = true) then match := match + 1; else match := 3; endif;
  endswitch;
 for tmpm: MessageType do msg_imm_sent_set[tmpm] := false; endfor;
  ...
  if (prevProcs != cur_state) then
    if (cur_idx <= 2) then
      if (cur_idx < 2 & cur_state = tar[cur_idx]) then
        cur_idx := cur_idx + 1;
      else 
        cur_idx := 3;
      endif;
