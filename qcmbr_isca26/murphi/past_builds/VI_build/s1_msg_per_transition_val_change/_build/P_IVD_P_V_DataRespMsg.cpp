/******************************
  Program "past_builds/VI_build/s1_msg_per_transition_val_change/out/P_IVD_P_V_DataRespMsg.m" compiled by "Caching Murphi Release 5.4.9.1"
...
#define PROTOCOL_NAME "past_builds/VI_build/s1_msg_per_transition_val_change/out/P_IVD_P_V_DataRespMsg"
...
/*** Variable declaration ***/
mu_1_Value mu_prevStateVal("prevStateVal",8);

/*** Variable declaration ***/
mu_1_ProcStateEnum mu_prevProcs("prevProcs",16);

/*** Variable declaration ***/
mu_1_Proc mu_selc("selc",24);

/*** Variable declaration ***/
mu_0_boolean mu_rec("rec",32);
...
bool mu__boolexpr7;
bool mu__boolexpr8;
  if (!(!(mu_prevProcs.isundefined()))) mu__boolexpr8 = FALSE ;
  else {
  mu__boolexpr8 = ((mu_prevProcs) == (mu_P_IVD)) ; 
}
  if (!(mu__boolexpr8)) mu__boolexpr7 = FALSE ;
  else {
  mu__boolexpr7 = ((mu_Procs[mu_selc].mu_state) == (mu_P_V)) ; 
}
  if (!(mu__boolexpr7)) mu__boolexpr6 = FALSE ;
  else {
  mu__boolexpr6 = (mu_rec) ; 
}
if ( mu__boolexpr6 )
{
if ( !((mu_prevStateVal) == (mu_Procs[mu_selc].mu_val)) ) Error.Error("Assertion failed: DIFF_VAL");
}
bool mu__boolexpr9;
  if (!(!(mu_prevProcs.isundefined()))) mu__boolexpr9 = FALSE ;
  else {
  mu__boolexpr9 = ((mu_prevProcs) != (mu_Procs[mu_selc].mu_state)) ; 
}
if ( mu__boolexpr9 )
{
mu_rec = mu_false;
...
bool mu__boolexpr10;
bool mu__boolexpr11;
  if (!(!(mu_curRequsts[mu_p].mu_vld.isundefined()))) mu__boolexpr11 = FALSE ;
  else {
  mu__boolexpr11 = (mu_curRequsts[mu_p].mu_vld) ; 
}
  if (!(mu__boolexpr11)) mu__boolexpr10 = FALSE ;
  else {
  mu__boolexpr10 = ((mu_curRequsts[mu_p].mu_tp) == (mu_store)) ; 
}
if ( mu__boolexpr10 )
{
...
for(int mu_n = 1; mu_n <= 6; mu_n++)
  if (( ( mu_n >= 1 ) && ( mu_n <= 3 ) )|| ( ( mu_n >= 6 ) && ( mu_n <= 6 ) )) {
bool mu__boolexpr12;
  if (!(mu_allAck)) mu__boolexpr12 = FALSE ;
  else {
  mu__boolexpr12 = (mu_ackBus[mu_n]) ; 
}
mu_allAck = mu__boolexpr12;
};
};
if ( mu_allAck )
{
...
bool mu__boolexpr13;
  ...
  void NextRule(unsigned & what_rule)
  {
	...
	if ( ( TRUE  ) ) {
bool mu__boolexpr14;
  if (!(!(mu_gBuf.mu_src.isundefined()))) mu__boolexpr14 = FALSE ;
  else {
...
bool mu__boolexpr16;
  ...
  mu__boolexpr15 = ((mu_gBuf.mu_mtype) == (mu_DataRespMsg)) ; 
}
if ( mu__boolexpr15 )
{
mu_rec = mu_true;
...
bool mu__boolexpr18;
  ...
  mu__boolexpr17 = ((mu_gBuf.mu_mtype) == (mu_DataRespMsg)) ; 
}
if ( mu__boolexpr17 )
{
mu_rec = mu_true;
