...
if (Procs[selc].state = P_V & prevProcVal != Procs[selc].val) then
  val_chk := true; 
  assert (Procs[selc].state != prevProcs) "NO PERMISSION STATE CHANGE?";
endif;
...
(msg_rec_set[GetMsg] = true & msg_rec_set[PutMsg] = false & msg_rec_set[DataRespMsg] = true)) ->
!(val_chk = true); 
-- violation: P_V is in the reachable_set and change value is possible
