/******************************
  Program "past_builds/VI_build/s3_2_reachable_set_val/out/P_I_ci_store_0_val_eq_req_for_P_IVD.m" compiled by "Caching Murphi Release 5.4.9.1"
...
#define PROTOCOL_NAME "past_builds/VI_build/s3_2_reachable_set_val/out/P_I_ci_store_0_val_eq_req_for_P_IVD"
...
mu_1_Value mu_prevStateVal("prevStateVal",0);

/*** Variable declaration ***/
mu_0_boolean mu_tracked("tracked",8);

/*** Variable declaration ***/
mu_0_boolean mu_start("start",16);

/*** Variable declaration ***/
mu_1__type_2 mu_cur_idx("cur_idx",24);

/*** Variable declaration ***/
mu_1__type_3 mu_tar("tar",32);
...
/*** Variable declaration ***/
mu_1_CoreReq mu_trackReq("trackReq",88);

/*** Variable declaration ***/
mu_0_boolean mu_val_chk("val_chk",112);

/*** Variable declaration ***/
mu_1_HomeState mu_HomeNode("HomeNode",120);

/*** Variable declaration ***/
mu_1__type_4 mu_Procs("Procs",136);
...
/*** Variable declaration ***/
mu_1__type_5 mu_ackBus("ackBus",232);

/*** Variable declaration ***/
mu_1__type_6 mu_msgBuf("msgBuf",264);

/*** Variable declaration ***/
mu_0_boolean mu_msgBufNonEmpty("msgBufNonEmpty",392);

/*** Variable declaration ***/
mu_1__type_7 mu_curRequsts("curRequsts",400);
...
if ( mu_start )
{
if ( (mu_Procs[mu_selc].mu_state) == (mu_P_IVD) )
{
if ( !(mu_trackReq.mu_vld) ) Error.Error("Assertion failed: not vld trackReq");
bool mu__boolexpr9;
  if (!((mu_trackReq.mu_tp) == (mu_ci_store))) mu__boolexpr9 = FALSE ;
  else {
  mu__boolexpr9 = ((mu_Procs[mu_selc].mu_val) == (mu_trackReq.mu_cl)) ; 
}
mu_val_chk = mu__boolexpr9;
}
...
{
bool mu__boolexpr10;
  if (!((mu_cur_idx) < (2))) mu__boolexpr10 = FALSE ;
  else {
  mu__boolexpr10 = ((mu_cur_state) == (mu_tar[mu_cur_idx])) ; 
}
if ( mu__boolexpr10 )
{
...
void world_class::print_diff( state *prevstate )
{
  if ( prevstate != NULL )
  {
    ...
    mu_trackReq.print_diff(prevstate);
...
bool mu__boolexpr16;
  ...
  mu__boolexpr13 = ((mu_prevProcReq.mu_tp) == (mu_ci_store)) ; 
}
...
bool mu__boolexpr20;
  ...
  mu__boolexpr17 = ((mu_prevProcReq.mu_tp) == (mu_ci_store)) ; 
}
	      if (mu__boolexpr17) {
		if ( ( TRUE  ) )
		  return;
...
mu_start = mu_true;
mu_tracked = mu_true;
mu_trackReq = mu_prevProcReq;
mu_trackReq.mu_vld = mu_true;
  };
...
bool mu__boolexpr24;
bool mu__boolexpr25;
  if (!((mu_p.mu_state) == (mu_P_V))) mu__boolexpr25 = FALSE ;
  else {
...
bool mu__boolexpr26;
bool mu__boolexpr27;
  if (!((mu_p.mu_state) == (mu_P_V))) mu__boolexpr27 = FALSE ;
  else {
...
bool mu__boolexpr28;
bool mu__boolexpr29;
  if (!((mu_p.mu_state) == (mu_P_I))) mu__boolexpr29 = FALSE ;
  else {
...
bool mu__boolexpr30;
bool mu__boolexpr31;
  if (!((mu_p.mu_state) == (mu_P_I))) mu__boolexpr31 = FALSE ;
  else {
...
/********************
  Invariant records
 ********************/
int mu__invariant_36() // Invariant "P_I_ci_store_rset_0"
{
bool mu__boolexpr37;
bool mu__boolexpr38;
bool mu__boolexpr39;
bool mu__boolexpr40;
  if (!(mu_tracked)) mu__boolexpr40 = FALSE ;
  else {
  mu__boolexpr40 = (!(mu_start)) ; 
}
  if (!(mu__boolexpr40)) mu__boolexpr39 = FALSE ;
  else {
  mu__boolexpr39 = ((mu_cur_idx) == (2)) ; 
}
  if (!(mu__boolexpr39)) mu__boolexpr38 = FALSE ;
  else {
  mu__boolexpr38 = (mu_true) ; 
}
  if (!(mu__boolexpr38)) mu__boolexpr37 = TRUE ;
  else {
  mu__boolexpr37 = ((mu_val_chk) == (mu_true)) ; 
}
return mu__boolexpr37;
};

bool mu__condition_41() // Condition for Rule "P_I_ci_store_rset_0"
{
...
int mu__invariant_42() // Invariant "swmr"
{
bool mu__quant43; 
mu__quant43 = TRUE;
{
...
bool mu__boolexpr45;
bool mu__boolexpr46;
  if (!((mu_c1) != (mu_c2))) mu__boolexpr46 = FALSE ;
  else {
  mu__boolexpr46 = ((mu_Procs[mu_c1].mu_state) == (mu_P_V)) ; 
}
  if (!(mu__boolexpr46)) mu__boolexpr45 = TRUE ;
  else {
  mu__boolexpr45 = ((mu_Procs[mu_c2].mu_state) != (mu_P_V)) ; 
}
if ( !(mu__boolexpr45) )
...
bool mu__condition_47() // Condition for Rule "swmr"
{
...
for(int mu_n = 1; mu_n <= 3; mu_n++) {
bool mu__boolexpr50;
  if (!((mu_Procs[mu_n].mu_state) == (mu_P_V))) mu__boolexpr50 = TRUE ;
  else {
...
int mu__invariant_52() // Invariant "value in memory matches value of last write, when invalid"
{
bool mu__boolexpr53;
  if (!((mu_HomeNode.mu_state) == (mu_H_I))) mu__boolexpr53 = TRUE ;
  else {
  mu__boolexpr53 = ((mu_MainMemory) == (mu_LastWrite)) ; 
}
return mu__boolexpr53;
};

bool mu__condition_54() // Condition for Rule "value in memory matches value of last write, when invalid"
{
...
if ( !(!((mu_Procs[mu_n].mu_state) == (mu_P_V))) )
  ...
  mu__boolexpr56 = (mu__quant57) ; 
}
return mu__boolexpr56;
};

bool mu__condition_58() // Condition for Rule "Invalid implies empty owner"
{
...
{"Invalid implies empty owner", &mu__condition_58, NULL, },
{"value in memory matches value of last write, when invalid", &mu__condition_54, NULL, },
{"values in valid state match last write", &mu__condition_51, NULL, },
{"swmr", &mu__condition_47, NULL, },
{"P_I_ci_store_rset_0", &mu__condition_41, NULL, },
};
...
void SymmetryClass::Heuristic_Fast_Canonicalize(state* s)
{
  ...
  mu_trackReq.SimpleCanonicalize(Perm);
