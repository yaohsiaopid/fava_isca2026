...
if (!isundefined(cur_node) & cur_node = selc) then prevSentMsg := gBuf.mtype; prevSentMsgVal := gBuf.val; endif;
End;
...
Begin
  ...
  msgBuf[src] := msg;
  msgBufNonEmpty := true;
if (!isundefined(cur_node) & cur_node = selc) then prevSentMsg := msg.mtype; prevSentMsgVal := msg.val; endif;
End;
...
invariant "ASSERT_P_V_rec_GetMsg"
  -- can P_V ever receives and proc GetMsg message and then send out 
  (!IsUndefined(prevProcs) & prevProcs = P_V &
    !isundefined(prevMsg) & prevMsg = GetMsg & 
    -- process or dst defined
    ((!isundefined(prevProcs) & prevProcs != Procs[selc].state) |
