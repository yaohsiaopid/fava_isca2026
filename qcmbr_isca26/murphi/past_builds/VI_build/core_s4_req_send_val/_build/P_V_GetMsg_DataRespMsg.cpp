/******************************
  Program "build/core_s4_req_send_val/out/P_V_GetMsg_DataRespMsg.m" compiled by "Caching Murphi Release 5.4.9.1"
...
#define PROTOCOL_NAME "build/core_s4_req_send_val/out/P_V_GetMsg_DataRespMsg"
...
/*** Variable declaration ***/
mu_1_Value mu_prevStateVal("prevStateVal",40);

/*** Variable declaration ***/
mu_1_ProcStateEnum mu_prevProcs("prevProcs",48);

/*** Variable declaration ***/
mu_1_Proc mu_selc("selc",56);

/*** Variable declaration ***/
mu_1_Value mu_prevSentMsgVal("prevSentMsgVal",64);

/*** Variable declaration ***/
mu_1_HomeState mu_HomeNode("HomeNode",72);

/*** Variable declaration ***/
mu_1__type_2 mu_Procs("Procs",88);

/*** Variable declaration ***/
mu_1_Value mu_LastWrite("LastWrite",136);

/*** Variable declaration ***/
mu_1_Value mu_MainMemory("MainMemory",144);

/*** Variable declaration ***/
mu_1_Message mu_gBuf("gBuf",152);

/*** Variable declaration ***/
mu_1__type_3 mu_ackBus("ackBus",184);

/*** Variable declaration ***/
mu_1__type_4 mu_msgBuf("msgBuf",216);

/*** Variable declaration ***/
mu_0_boolean mu_msgBufNonEmpty("msgBufNonEmpty",344);

/*** Variable declaration ***/
mu_1__type_5 mu_curRequsts("curRequsts",352);
...
mu_prevMsg.undefine();
if (mu_Procs[mu_selc].mu_val.isundefined())
  mu_prevStateVal.undefine();
else
  mu_prevStateVal = mu_Procs[mu_selc].mu_val;
if (mu_Procs[mu_selc].mu_state.isundefined())
  mu_prevProcs.undefine();
else
  mu_prevProcs = mu_Procs[mu_selc].mu_state;
mu_prevSentMsgVal.undefine();
};
...
bool mu__boolexpr6;
  if (!(!(mu_cur_node.isundefined()))) mu__boolexpr6 = FALSE ;
  else {
  mu__boolexpr6 = ((mu_cur_node) == (mu_selc)) ; 
}
if ( mu__boolexpr6 )
{
if (mu_gBuf.mu_mtype.isundefined())
  ...
  mu_prevSentMsg = mu_gBuf.mu_mtype;
if (mu_gBuf.mu_val.isundefined())
  mu_prevSentMsgVal.undefine();
else
  mu_prevSentMsgVal = mu_gBuf.mu_val;
}
};
/*** end procedure declaration ***/
...
if (mu_msg.mu_mtype.isundefined())
  ...
  mu_prevSentMsg = mu_msg.mu_mtype;
if (mu_msg.mu_val.isundefined())
  mu_prevSentMsgVal.undefine();
else
  mu_prevSentMsgVal = mu_msg.mu_val;
}
};
/*** end procedure declaration ***/
...
bool mu__boolexpr8;
bool mu__boolexpr9;
  if (!(!(mu_curRequsts[mu_p].mu_vld.isundefined()))) mu__boolexpr9 = FALSE ;
  else {
  mu__boolexpr9 = (mu_curRequsts[mu_p].mu_vld) ; 
}
  if (!(mu__boolexpr9)) mu__boolexpr8 = FALSE ;
  else {
  mu__boolexpr8 = ((mu_curRequsts[mu_p].mu_tp) == (mu_store)) ; 
}
if ( mu__boolexpr8 )
{
...
void world_class::clear()
{
  ...
  mu_prevMsgSrc.clear();
  mu_prevMsg.clear();
  mu_prevStateVal.clear();
  mu_prevProcs.clear();
  mu_selc.clear();
  mu_prevSentMsgVal.clear();
  mu_HomeNode.clear();
  mu_Procs.clear();
  mu_LastWrite.clear();
  mu_MainMemory.clear();
...
void world_class::reset()
{
  ...
  mu_prevMsgSrc.reset();
  mu_prevMsg.reset();
  mu_prevStateVal.reset();
  mu_prevProcs.reset();
  mu_selc.reset();
  mu_prevSentMsgVal.reset();
  mu_HomeNode.reset();
  mu_Procs.reset();
  mu_LastWrite.reset();
  mu_MainMemory.reset();
...
void world_class::print()
{
  ...
  mu_prevMsgSrc.print();
  mu_prevMsg.print();
  mu_prevStateVal.print();
  mu_prevProcs.print();
  mu_selc.print();
  mu_prevSentMsgVal.print();
  mu_HomeNode.print();
  mu_Procs.print();
  mu_LastWrite.print();
  mu_MainMemory.print();
...
void world_class::print_statistic()
{
  ...
  mu_prevStateVal.print_statistic();
  mu_prevProcs.print_statistic();
  mu_selc.print_statistic();
  mu_prevSentMsgVal.print_statistic();
...
void world_class::print_diff( state *prevstate )
{
  if ( prevstate != NULL )
  {
    ...
    mu_prevStateVal.print_diff(prevstate);
    mu_prevProcs.print_diff(prevstate);
    mu_selc.print_diff(prevstate);
    mu_prevSentMsgVal.print_diff(prevstate);
...
void world_class::to_state(state *newstate)
{
  ...
  mu_prevStateVal.to_state( newstate );
  mu_prevProcs.to_state( newstate );
  mu_selc.to_state( newstate );
  mu_prevSentMsgVal.to_state( newstate );
...
for(int mu_n = 1; mu_n <= 6; mu_n++)
  if (( ( mu_n >= 1 ) && ( mu_n <= 3 ) )|| ( ( mu_n >= 6 ) && ( mu_n <= 6 ) )) {
bool mu__boolexpr10;
  if (!(mu_allAck)) mu__boolexpr10 = FALSE ;
  else {
  mu__boolexpr10 = (mu_ackBus[mu_n]) ; 
}
mu_allAck = mu__boolexpr10;
};
};
if ( mu_allAck )
{
...
public:
  ...
  bool Condition(unsigned r)
  {
    ...
    r = r / 4;
bool mu__boolexpr11;
  if (!(!(mu_gBuf.mu_src.isundefined()))) mu__boolexpr11 = FALSE ;
  else {
...
bool mu__boolexpr17;
bool mu__boolexpr18;
  if (!((mu_p.mu_state) == (mu_P_V))) mu__boolexpr18 = FALSE ;
  else {
...
/********************
  Invariant records
 ********************/
int mu__invariant_27() // Invariant "ASSERT_P_V_rec_GetMsg"
{
bool mu__boolexpr28;
bool mu__boolexpr29;
bool mu__boolexpr30;
...
bool mu__boolexpr37;
  if (!(!(mu_prevSentMsgVal.isundefined()))) mu__boolexpr37 = FALSE ;
  else {
  mu__boolexpr37 = ((mu_prevSentMsgVal) == (mu_prevStateVal)) ; 
}
  mu__boolexpr28 = (mu__boolexpr37) ; 
}
return mu__boolexpr28;
};

bool mu__condition_38() // Condition for Rule "ASSERT_P_V_rec_GetMsg"
{
...
int mu__invariant_39() // Invariant "swmr"
{
bool mu__quant40; 
mu__quant40 = TRUE;
{
...
bool mu__boolexpr42;
bool mu__boolexpr43;
  if (!((mu_c1) != (mu_c2))) mu__boolexpr43 = FALSE ;
  else {
  mu__boolexpr43 = ((mu_Procs[mu_c1].mu_state) == (mu_P_V)) ; 
}
  if (!(mu__boolexpr43)) mu__boolexpr42 = TRUE ;
  else {
  mu__boolexpr42 = ((mu_Procs[mu_c2].mu_state) != (mu_P_V)) ; 
}
if ( !(mu__boolexpr42) )
...
bool mu__condition_44() // Condition for Rule "swmr"
{
...
for(int mu_n = 1; mu_n <= 3; mu_n++) {
bool mu__boolexpr47;
  if (!((mu_Procs[mu_n].mu_state) == (mu_P_V))) mu__boolexpr47 = TRUE ;
  else {
...
bool mu__condition_48() // Condition for Rule "values in valid state match last write"
{
...
int mu__invariant_49() // Invariant "value in memory matches value of last write, when invalid"
{
bool mu__boolexpr50;
  if (!((mu_HomeNode.mu_state) == (mu_H_I))) mu__boolexpr50 = TRUE ;
  else {
  mu__boolexpr50 = ((mu_MainMemory) == (mu_LastWrite)) ; 
}
return mu__boolexpr50;
};

bool mu__condition_51() // Condition for Rule "value in memory matches value of last write, when invalid"
{
...
int mu__invariant_52() // Invariant "Invalid implies empty owner"
{
bool mu__boolexpr53;
  if (!((mu_HomeNode.mu_state) == (mu_H_I))) mu__boolexpr53 = TRUE ;
  else {
bool mu__quant54; 
mu__quant54 = TRUE;
{
...
{"Invalid implies empty owner", &mu__condition_55, NULL, },
{"value in memory matches value of last write, when invalid", &mu__condition_51, NULL, },
{"values in valid state match last write", &mu__condition_48, NULL, },
{"swmr", &mu__condition_44, NULL, },
{"ASSERT_P_V_rec_GetMsg", &mu__condition_38, NULL, },
};
  ...
  if (args->symmetry_reduction.value)
    {
      if (  args->sym_alg.mode == argsym_alg::Exhaustive_Fast_Canonicalize
          ...
          if (Perm.In(i))
            {
              ...
              if (args->multiset_reduction.value)
                mu_LastWrite.MultisetSort();
              mu_prevSentMsgVal.Permute(Perm,i);
              if (args->multiset_reduction.value)
                mu_prevSentMsgVal.MultisetSort();
              ...
              if (args->multiset_reduction.value)
                mu_prevMsgSrc.MultisetSort();
              mu_prevStateVal.Permute(Perm,i);
              if (args->multiset_reduction.value)
                mu_prevStateVal.MultisetSort();
        ...
        if (ns != workingstate)
          ...
          if (args->multiset_reduction.value)
            mu_LastWrite.MultisetSort();
          mu_prevSentMsgVal.Permute(Perm,0);
          if (args->multiset_reduction.value)
            mu_prevSentMsgVal.MultisetSort();
          ...
          if (args->multiset_reduction.value)
            mu_prevMsgSrc.MultisetSort();
          mu_prevStateVal.Permute(Perm,0);
          if (args->multiset_reduction.value)
            mu_prevStateVal.MultisetSort();
...
void SymmetryClass::Heuristic_Fast_Canonicalize(state* s)
{
  ...
  mu_prevSentMsgVal.SimpleCanonicalize(Perm);
  ...
  mu_prevStateVal.SimpleCanonicalize(Perm);
