...
-- The request that the randomly selected processor is currently processing
  ...
  reached_set: multiset [6] of s_cache;
  -- TODO: size to be maximum number of message than can be sent/received in
  -- single transition
  cur_node: Machines;  

  prevMsg : MessageType;
  prevMsg_chk: MessageType;
...
begin
  cur_state := i_cache[selc].CL[myaddr].State;
  in_stable_state := (cur_state = cache_I | cur_state = cache_S | cur_state = cache_M);
  if (start) then
    -- BLOCK_WITHIN_START
...
-- --     -- FVT {
--    if (m = selc) then
--      assert (pendingReq.vld) "no pending req?";
...
--   -- FVT
  ...
  msg_sent_set[m] := false;
  msg_rec_set[m] := false;
  endfor;
  undefine prevMsg;
  undefine prevMsg_chk;
endstartstate;
