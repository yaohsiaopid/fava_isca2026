...
-- The request that the randomly selected processor is currently processing
  ...
  prevProcs: ENTRY_cache; -- STATE_TRANSITION 
...
begin
  ...
  if (start) then
    ...
    if (!in_stable_state) then
      if (prevProcs.State != cur_state) then
  ...
  -- MSG_CHK cur_node := m; 
  prevProcs := i_cache[selc].CL[myaddr];  -- STATE_TRANSITION   -- other data are assoc. w/ this prevState
