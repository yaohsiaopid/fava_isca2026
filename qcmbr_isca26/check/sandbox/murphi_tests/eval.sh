if [ $1 == "run" ]; then
mkdir -p results_new
../../util/runtests.sh -m  ../../../murphi/artifact/vi_buggy_test.uarch -p ../../tests/coherence_augmented
column -s, -t < results_new/latest/vi_buggy_test.csv
fi

if [ $1 == "run_fix" ]; then
mkdir -p results_new
../../util/runtests.sh -m  ../../../murphi/artifact/vi_test.uarch -p ../../tests/coherence_augmented
