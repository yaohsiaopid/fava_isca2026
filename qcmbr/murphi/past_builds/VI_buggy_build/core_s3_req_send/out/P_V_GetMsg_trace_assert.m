...
selc: Proc;
prevSentMsgSet: array [MessageType] of boolean;
...
if (!isundefined(cur_node) & cur_node = selc) then prevSentMsgSet[gBuf.mtype]:= true; endif;
End;
...
Begin
  ...
  msgBuf[src] := msg;
  msgBufNonEmpty := true;
if (!isundefined(cur_node) & cur_node = selc) then prevSentMsgSet[msg.mtype]:= true; endif;
End;
...
invariant "P_V_rec_GetMsg"
  -- can P_V ever receives and proc GetMsg message 
  (!IsUndefined(prevProcs) & prevProcs = P_V &
    !isundefined(prevMsg) & prevMsg = GetMsg) ->
     (prevSentMsgSet[GetMsg] = false & prevSentMsgSet[PutMsg] = false & prevSentMsgSet[DataRespMsg] = true) 
;
