...
var
cur_node: Node;
msg_imm_rec_set: array [MessageType] of boolean;
...
begin
if (!isundefined(prevProcs) & prevProcs != Procs[selc].state) then
for m:MessageType do msg_imm_rec_set[m] := false;
endfor;
endif;
...
ruleset n:Node Do
 ...
 book_keep(n);
  ...
  if (!isundefined(cur_node) & cur_node = selc) then
    msg_imm_rec_set[gBuf.mtype] := true;
  endif;
          HomeReceive(gBuf);
      else
...
invariant "P_I_to_P_IVD"
  ((!IsUndefined(prevProcs) & (prevProcs = P_I))
  & (Procs[selc].state = P_IVD)) ->
  ((msg_imm_rec_set[GetMsg] = false & msg_imm_rec_set[PutMsg] = false & msg_imm_rec_set[DataRespMsg] = false) | (msg_imm_rec_set[GetMsg] = true & msg_imm_rec_set[PutMsg] = false & msg_imm_rec_set[DataRespMsg] = true) | (msg_imm_rec_set[GetMsg] = false & msg_imm_rec_set[PutMsg] = true & msg_imm_rec_set[DataRespMsg] = false) | (msg_imm_rec_set[GetMsg] = true & msg_imm_rec_set[PutMsg] = true & msg_imm_rec_set[DataRespMsg] = true) | (msg_imm_rec_set[GetMsg] = false & msg_imm_rec_set[PutMsg] = false & msg_imm_rec_set[DataRespMsg] = true) | (msg_imm_rec_set[GetMsg] = false & msg_imm_rec_set[PutMsg] = true & msg_imm_rec_set[DataRespMsg] = true));
