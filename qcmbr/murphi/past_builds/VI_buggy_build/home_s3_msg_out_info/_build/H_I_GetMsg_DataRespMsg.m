...
home_val := HomeNode.val;sent := false;
...
if (!isundefined(cur_node) & cur_node = selh & gBuf.mtype = DataRespMsg) then
 sent := true;
 endif;
 if (!isundefined(cur_node) & cur_node = selh & gBuf.mtype = DataRespMsg) then
 val_match := (gBuf.val = home_val);
 endif;
End;
...
Begin
  ...
  msgBuf[src] := msg;
  msgBufNonEmpty := true;
if (!isundefined(cur_node) & cur_node = selh & msg.mtype = DataRespMsg) then
 sent := true;
 endif;
 if (!isundefined(cur_node) & cur_node = selh & msg.mtype = DataRespMsg) then
 val_match := (msg.val = home_val);
 endif;
End;
...
invariant "ASSERT_H_I_accept_req_GetMsg"
  (!isundefined(prevHomeNode) & prevHomeNode = H_I & 
   !isundefined(prevRecProcMsg) & prevRecProcMsg = GetMsg & sent) ->
   val_match;
