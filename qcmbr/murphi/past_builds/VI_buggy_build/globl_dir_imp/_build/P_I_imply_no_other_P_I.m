...
invariant "P_I_imply_not_P_I_others"
forall n1: Proc do
  ((forall tmpn: Proc do
  (Procs[tmpn].state = P_I | Procs[tmpn].state = P_V)
endforall) & (forall d: Home do
  (HomeNode.state = H_I | HomeNode.state = H_V)
endforall) & Procs[n1].state = P_I) ->
  (forall n2: Proc do
    n2 != n1 ->
    Procs[n2].state != P_I
  endforall)
