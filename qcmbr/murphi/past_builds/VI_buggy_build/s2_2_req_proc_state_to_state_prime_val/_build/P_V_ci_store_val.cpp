/******************************
  Program "past_builds/VI_buggy_build/s2_2_req_proc_state_to_state_prime_val/out/P_V_ci_store_val.m" compiled by "Caching Murphi Release 5.4.9.1"
...
#define PROTOCOL_NAME "past_builds/VI_buggy_build/s2_2_req_proc_state_to_state_prime_val/out/P_V_ci_store_val"
#define BITS_IN_WORLD 440
#define ALIGN
...
mu_1_Value mu_prevStateVal_2("prevStateVal_2",0);

/*** Variable declaration ***/
mu_1_CoreReq mu_prevProcReq_2("prevProcReq_2",8);

/*** Variable declaration ***/
mu_1_ProcStateEnum mu_prevProcs_2("prevProcs_2",32);
...
/*** Variable declaration ***/
mu_1_CoreReq mu_prevProcReq("prevProcReq",56);

/*** Variable declaration ***/
mu_1_Proc mu_selc("selc",80);

/*** Variable declaration ***/
mu_1_HomeState mu_HomeNode("HomeNode",88);

/*** Variable declaration ***/
mu_1__type_2 mu_Procs("Procs",104);

/*** Variable declaration ***/
mu_1_Value mu_LastWrite("LastWrite",152);

/*** Variable declaration ***/
mu_1_Value mu_MainMemory("MainMemory",160);

/*** Variable declaration ***/
mu_1_Message mu_gBuf("gBuf",168);

/*** Variable declaration ***/
mu_1__type_3 mu_ackBus("ackBus",200);

/*** Variable declaration ***/
mu_1__type_4 mu_msgBuf("msgBuf",232);

/*** Variable declaration ***/
mu_0_boolean mu_msgBufNonEmpty("msgBufNonEmpty",360);

/*** Variable declaration ***/
mu_1__type_5 mu_curRequsts("curRequsts",368);

void mu_book_keep(const mu_1_Node& mu_m)
{
if (mu_prevStateVal.isundefined())
  mu_prevStateVal_2.undefine();
else
  mu_prevStateVal_2 = mu_prevStateVal;
mu_prevProcReq_2 = mu_prevProcReq;
if (mu_prevProcs.isundefined())
  mu_prevProcs_2.undefine();
else
  mu_prevProcs_2 = mu_prevProcs;
...
{
  mu_prevStateVal_2.clear();
  mu_prevProcReq_2.clear();
  mu_prevProcs_2.clear();
...
{
  mu_prevStateVal_2.reset();
  mu_prevProcReq_2.reset();
  mu_prevProcs_2.reset();
...
void world_class::print()
{
  ...
  if ( num_calls == 0 ) {
    num_calls++;
  mu_prevStateVal_2.print();
  mu_prevProcReq_2.print();
  mu_prevProcs_2.print();
...
void world_class::print_statistic()
{
  ...
  if ( num_calls == 0 ) {
    num_calls++;
  mu_prevStateVal_2.print_statistic();
  mu_prevProcReq_2.print_statistic();
  mu_prevProcs_2.print_statistic();
...
void world_class::print_diff( state *prevstate )
{
  if ( prevstate != NULL )
  {
    mu_prevStateVal_2.print_diff(prevstate);
    mu_prevProcReq_2.print_diff(prevstate);
    mu_prevProcs_2.print_diff(prevstate);
...
{
  mu_prevStateVal_2.to_state( newstate );
  mu_prevProcReq_2.to_state( newstate );
  mu_prevProcs_2.to_state( newstate );
...
for(int mu_n = 1; mu_n <= 6; mu_n++)
  if (( ( mu_n >= 1 ) && ( mu_n <= 3 ) )|| ( ( mu_n >= 6 ) && ( mu_n <= 6 ) )) {
bool mu__boolexpr6;
  if (!(mu_allAck)) mu__boolexpr6 = FALSE ;
  else {
  mu__boolexpr6 = (mu_ackBus[mu_n]) ; 
}
mu_allAck = mu__boolexpr6;
};
};
if ( mu_allAck )
{
...
public:
  ...
  bool Condition(unsigned r)
  {
    ...
    r = r / 4;
bool mu__boolexpr7;
  if (!(!(mu_gBuf.mu_src.isundefined()))) mu__boolexpr7 = FALSE ;
  else {
  mu__boolexpr7 = (!(mu_ackBus[mu_n])) ; 
}
  ...
  void NextRule(unsigned & what_rule)
  {
	...
	if ( ( TRUE  ) ) {
bool mu__boolexpr8;
  if (!(!(mu_gBuf.mu_src.isundefined()))) mu__boolexpr8 = FALSE ;
  else {
...
bool mu__boolexpr9;
bool mu__boolexpr10;
  if (!((mu_p.mu_state) == (mu_P_V))) mu__boolexpr10 = FALSE ;
  else {
  mu__boolexpr10 = (!(mu_msgBufNonEmpty)) ; 
}
  if (!(mu__boolexpr10)) mu__boolexpr9 = FALSE ;
  else {
  mu__boolexpr9 = (mu_gBuf.mu_src.isundefined()) ; 
}
...
bool mu__boolexpr11;
bool mu__boolexpr12;
  if (!((mu_p.mu_state) == (mu_P_V))) mu__boolexpr12 = FALSE ;
  else {
  mu__boolexpr12 = (!(mu_msgBufNonEmpty)) ; 
}
  if (!(mu__boolexpr12)) mu__boolexpr11 = FALSE ;
  else {
  mu__boolexpr11 = (mu_gBuf.mu_src.isundefined()) ; 
}
	      if (mu__boolexpr11) {
		if ( ( TRUE  ) )
		  return;
...
bool mu__boolexpr13;
bool mu__boolexpr14;
  if (!((mu_p.mu_state) == (mu_P_I))) mu__boolexpr14 = FALSE ;
  else {
...
bool mu__boolexpr15;
bool mu__boolexpr16;
  if (!((mu_p.mu_state) == (mu_P_I))) mu__boolexpr16 = FALSE ;
  else {
...
/********************
  Invariant records
 ********************/
int mu__invariant_21() // Invariant "P_V_accept_and_perform_ci_store_value_change"
{
bool mu__boolexpr22;
bool mu__boolexpr23;
bool mu__boolexpr24;
bool mu__boolexpr25;
  if (!(!(mu_prevProcs_2.isundefined()))) mu__boolexpr25 = FALSE ;
  else {
  mu__boolexpr25 = ((mu_prevProcs_2) == (mu_P_V)) ; 
}
  if (!(mu__boolexpr25)) mu__boolexpr24 = FALSE ;
  else {
  mu__boolexpr24 = (!(mu_prevProcReq_2.mu_tp.isundefined())) ; 
}
  if (!(mu__boolexpr24)) mu__boolexpr23 = FALSE ;
  else {
  mu__boolexpr23 = ((mu_prevProcReq_2.mu_tp) == (mu_ci_store)) ; 
}
...
bool mu__boolexpr27;
bool mu__boolexpr28;
  if (mu_prevStateVal.isundefined()) mu__boolexpr28 = TRUE ;
  else {
  mu__boolexpr28 = ((mu_prevStateVal) == (mu_prevStateVal_2)) ; 
}
...
bool mu__condition_29() // Condition for Rule "P_V_accept_and_perform_ci_store_value_change"
{
...
int mu__invariant_30() // Invariant "swmr"
{
bool mu__quant31; 
mu__quant31 = TRUE;
{
...
bool mu__boolexpr33;
bool mu__boolexpr34;
  if (!((mu_c1) != (mu_c2))) mu__boolexpr34 = FALSE ;
  else {
  mu__boolexpr34 = ((mu_Procs[mu_c1].mu_state) == (mu_P_V)) ; 
}
  if (!(mu__boolexpr34)) mu__boolexpr33 = TRUE ;
  else {
  mu__boolexpr33 = ((mu_Procs[mu_c2].mu_state) != (mu_P_V)) ; 
}
if ( !(mu__boolexpr33) )
...
bool mu__condition_35() // Condition for Rule "swmr"
{
...
int mu__invariant_36() // Invariant "values in valid state match last write"
{
bool mu__quant37; 
mu__quant37 = TRUE;
{
for(int mu_n = 1; mu_n <= 3; mu_n++) {
bool mu__boolexpr38;
  if (!((mu_Procs[mu_n].mu_state) == (mu_P_V))) mu__boolexpr38 = TRUE ;
  else {
  mu__boolexpr38 = ((mu_Procs[mu_n].mu_val) == (mu_LastWrite)) ; 
}
if ( !(mu__boolexpr38) )
...
bool mu__condition_39() // Condition for Rule "values in valid state match last write"
{
...
int mu__invariant_40() // Invariant "value in memory matches value of last write, when invalid"
{
bool mu__boolexpr41;
  if (!((mu_HomeNode.mu_state) == (mu_H_I))) mu__boolexpr41 = TRUE ;
  else {
  mu__boolexpr41 = ((mu_MainMemory) == (mu_LastWrite)) ; 
}
return mu__boolexpr41;
};

bool mu__condition_42() // Condition for Rule "value in memory matches value of last write, when invalid"
{
...
int mu__invariant_43() // Invariant "Invalid implies empty owner"
{
bool mu__boolexpr44;
...
if ( !(!((mu_Procs[mu_n].mu_state) == (mu_P_V))) )
  ...
  mu__boolexpr44 = (mu__quant45) ; 
}
return mu__boolexpr44;
};

bool mu__condition_46() // Condition for Rule "Invalid implies empty owner"
{
...
{"Invalid implies empty owner", &mu__condition_46, NULL, },
{"value in memory matches value of last write, when invalid", &mu__condition_42, NULL, },
{"values in valid state match last write", &mu__condition_39, NULL, },
{"swmr", &mu__condition_35, NULL, },
{"P_V_accept_and_perform_ci_store_value_change", &mu__condition_29, NULL, },
};
  ...
  if (args->symmetry_reduction.value)
    {
      if (  args->sym_alg.mode == argsym_alg::Exhaustive_Fast_Canonicalize
          ...
          if (Perm.In(i))
            {
              ...
              if (args->multiset_reduction.value)
                mu_selc.MultisetSort();
              mu_prevStateVal_2.Permute(Perm,i);
              if (args->multiset_reduction.value)
                mu_prevStateVal_2.MultisetSort();
              mu_prevProcReq_2.Permute(Perm,i);
              if (args->multiset_reduction.value)
                mu_prevProcReq_2.MultisetSort();
        ...
        if (ns != workingstate)
          ...
          if (args->multiset_reduction.value)
            mu_selc.MultisetSort();
          mu_prevStateVal_2.Permute(Perm,0);
          if (args->multiset_reduction.value)
            mu_prevStateVal_2.MultisetSort();
          mu_prevProcReq_2.Permute(Perm,0);
          if (args->multiset_reduction.value)
            mu_prevProcReq_2.MultisetSort();
...
void SymmetryClass::Heuristic_Fast_Canonicalize(state* s)
{
  ...
  mu_prevStateVal_2.SimpleCanonicalize(Perm);

  mu_prevProcReq_2.SimpleCanonicalize(Perm);
