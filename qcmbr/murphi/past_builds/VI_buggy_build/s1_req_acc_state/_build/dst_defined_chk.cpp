/******************************
  Program "past_builds/VI_buggy_build/s1_req_acc_state/out/dst_defined_chk.m" compiled by "Caching Murphi Release 5.4.9.1"
...
#define PROTOCOL_NAME "past_builds/VI_buggy_build/s1_req_acc_state/out/dst_defined_chk"
...
/*** Variable declaration ***/
mu_1_HomeState mu_HomeNode("HomeNode",8);

/*** Variable declaration ***/
mu_1__type_2 mu_Procs("Procs",24);

/*** Variable declaration ***/
mu_1_Value mu_LastWrite("LastWrite",72);

/*** Variable declaration ***/
mu_1_Value mu_MainMemory("MainMemory",80);

/*** Variable declaration ***/
mu_1_Message mu_gBuf("gBuf",88);

/*** Variable declaration ***/
mu_1__type_3 mu_ackBus("ackBus",120);

/*** Variable declaration ***/
mu_1__type_4 mu_msgBuf("msgBuf",152);

/*** Variable declaration ***/
mu_0_boolean mu_msgBufNonEmpty("msgBufNonEmpty",280);

/*** Variable declaration ***/
mu_1__type_5 mu_curRequsts("curRequsts",288);
...
if ( !(!(mu_gBuf.mu_dst.isundefined())) ) Error.Error("Assertion failed: DST NOT DEFINED");
};
/*** end procedure declaration ***/
...
if ( !(!(mu_msg.mu_dst.isundefined())) ) Error.Error("Assertion failed: DST NOT DEFINED");
};
/*** end procedure declaration ***/
...
/********************
  Invariant records
 ********************/
int mu__invariant_21() // Invariant "swmr"
{
bool mu__quant22; 
mu__quant22 = TRUE;
{
...
bool mu__boolexpr24;
bool mu__boolexpr25;
  if (!((mu_c1) != (mu_c2))) mu__boolexpr25 = FALSE ;
  else {
  mu__boolexpr25 = ((mu_Procs[mu_c1].mu_state) == (mu_P_V)) ; 
}
  if (!(mu__boolexpr25)) mu__boolexpr24 = TRUE ;
  else {
  mu__boolexpr24 = ((mu_Procs[mu_c2].mu_state) != (mu_P_V)) ; 
}
if ( !(mu__boolexpr24) )
...
bool mu__condition_26() // Condition for Rule "swmr"
{
...
bool mu__boolexpr29;
  if (!((mu_Procs[mu_n].mu_state) == (mu_P_V))) mu__boolexpr29 = TRUE ;
  else {
...
bool mu__condition_30() // Condition for Rule "values in valid state match last write"
{
...
int mu__invariant_31() // Invariant "value in memory matches value of last write, when invalid"
{
bool mu__boolexpr32;
  if (!((mu_HomeNode.mu_state) == (mu_H_I))) mu__boolexpr32 = TRUE ;
  else {
  mu__boolexpr32 = ((mu_MainMemory) == (mu_LastWrite)) ; 
}
return mu__boolexpr32;
};

bool mu__condition_33() // Condition for Rule "value in memory matches value of last write, when invalid"
{
...
int mu__invariant_34() // Invariant "Invalid implies empty owner"
{
bool mu__boolexpr35;
  if (!((mu_HomeNode.mu_state) == (mu_H_I))) mu__boolexpr35 = TRUE ;
  else {
bool mu__quant36; 
mu__quant36 = TRUE;
{
...
{"Invalid implies empty owner", &mu__condition_37, NULL, },
{"value in memory matches value of last write, when invalid", &mu__condition_33, NULL, },
{"values in valid state match last write", &mu__condition_30, NULL, },
{"swmr", &mu__condition_26, NULL, },
};
