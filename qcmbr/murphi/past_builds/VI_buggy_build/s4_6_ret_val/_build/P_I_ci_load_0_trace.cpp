/******************************
  Program "past_builds/VI_buggy_build/s4_6_ret_val/out/P_I_ci_load_0_trace.m" compiled by "Caching Murphi Release 5.4.9.1"
...
#define PROTOCOL_NAME "past_builds/VI_buggy_build/s4_6_ret_val/out/P_I_ci_load_0_trace"
#define BITS_IN_WORLD 480
#define ALIGN
...
/*** Variable declaration ***/
mu_1__type_2 mu_reached_set("reached_set",24);
...
/*** Variable declaration ***/
mu_1_ProcStateEnum mu_state_ret_val("state_ret_val",112);

/*** Variable declaration ***/
mu_0_boolean mu_just_ret("just_ret",120);

/*** Variable declaration ***/
mu_1_HomeState mu_HomeNode("HomeNode",128);

/*** Variable declaration ***/
mu_1__type_3 mu_Procs("Procs",144);

/*** Variable declaration ***/
mu_1_Value mu_LastWrite("LastWrite",192);

/*** Variable declaration ***/
mu_1_Value mu_MainMemory("MainMemory",200);

/*** Variable declaration ***/
mu_1_Message mu_gBuf("gBuf",208);

/*** Variable declaration ***/
mu_1__type_4 mu_ackBus("ackBus",240);

/*** Variable declaration ***/
mu_1__type_5 mu_msgBuf("msgBuf",272);

/*** Variable declaration ***/
mu_0_boolean mu_msgBufNonEmpty("msgBufNonEmpty",400);

/*** Variable declaration ***/
mu_1__type_6 mu_curRequsts("curRequsts",408);
...
bool mu__boolexpr7;
  if ((mu_cur_state) == (mu_P_I)) mu__boolexpr7 = TRUE ;
  else {
  mu__boolexpr7 = ((mu_cur_state) == (mu_P_V)) ; 
}
mu_in_stable_state = mu__boolexpr7;
if ( mu_start )
{
...
/*** begin multisetcount 0 declaration ***/
  ...
  for (mu_i = 0; ; mu_i=mu_i+1)
	...
	{
	  if ( (mu_reached_set[mu_i]) == (mu_cur_state) ) mu__intexpr8++;
	}
      if (mu_i == 3-1) break;
    }
  }
...
{
if (mu_Procs[mu_selc].mu_state.isundefined())
  mu_state_ret_val.undefine();
else
  mu_state_ret_val = mu_Procs[mu_selc].mu_state;
}
mu_just_ret = mu_false;
};
...
bool mu__boolexpr9;
bool mu__boolexpr10;
  if (!(mu_start)) mu__boolexpr10 = FALSE ;
  else {
  mu__boolexpr10 = (!(mu_cur_node.isundefined())) ; 
}
  if (!(mu__boolexpr10)) mu__boolexpr9 = FALSE ;
  else {
  mu__boolexpr9 = ((mu_cur_node) == (mu_selc)) ; 
}
if ( mu__boolexpr9 )
{
if ( !(mu_state_ret_val.isundefined()) )
{
mu_state_ret_val.undefine();
}
else
...
void world_class::print_statistic()
{
  ...
  mu_state_ret_val.print_statistic();
  mu_just_ret.print_statistic();
...
void world_class::print_diff( state *prevstate )
{
  if ( prevstate != NULL )
  {
    ...
    mu_state_ret_val.print_diff(prevstate);
    mu_just_ret.print_diff(prevstate);
...
void world_class::to_state(state *newstate)
{
  ...
  mu_state_ret_val.to_state( newstate );
  mu_just_ret.to_state( newstate );
...
bool mu__boolexpr13;
bool mu__boolexpr14;
  if (!(!(mu_tracked))) mu__boolexpr14 = FALSE ;
  else {
  mu__boolexpr14 = (!(mu_prevProcs.isundefined())) ; 
}
  if (!(mu__boolexpr14)) mu__boolexpr13 = FALSE ;
  else {
  mu__boolexpr13 = ((mu_prevProcs) == (mu_P_I)) ; 
}
  if (!(mu__boolexpr13)) mu__boolexpr12 = FALSE ;
  else {
  mu__boolexpr12 = (!(mu_prevProcReq.mu_tp.isundefined())) ; 
}
  if (!(mu__boolexpr12)) mu__boolexpr11 = FALSE ;
  else {
  mu__boolexpr11 = ((mu_prevProcReq.mu_tp) == (mu_ci_load)) ; 
}
...
bool mu__boolexpr17;
bool mu__boolexpr18;
  if (!(!(mu_tracked))) mu__boolexpr18 = FALSE ;
  else {
  mu__boolexpr18 = (!(mu_prevProcs.isundefined())) ; 
}
  if (!(mu__boolexpr18)) mu__boolexpr17 = FALSE ;
  else {
  mu__boolexpr17 = ((mu_prevProcs) == (mu_P_I)) ; 
}
  if (!(mu__boolexpr17)) mu__boolexpr16 = FALSE ;
  else {
  mu__boolexpr16 = (!(mu_prevProcReq.mu_tp.isundefined())) ; 
}
  if (!(mu__boolexpr16)) mu__boolexpr15 = FALSE ;
  else {
  mu__boolexpr15 = ((mu_prevProcReq.mu_tp) == (mu_ci_load)) ; 
}
	      if (mu__boolexpr15) {
		if ( ( TRUE  ) )
		  return;
...
for(int mu_n = 1; mu_n <= 6; mu_n++)
  if (( ( mu_n >= 1 ) && ( mu_n <= 3 ) )|| ( ( mu_n >= 6 ) && ( mu_n <= 6 ) )) {
bool mu__boolexpr19;
  if (!(mu_allAck)) mu__boolexpr19 = FALSE ;
  else {
  mu__boolexpr19 = (mu_ackBus[mu_n]) ; 
}
mu_allAck = mu__boolexpr19;
};
};
if ( mu_allAck )
{
...
public:
  ...
  bool Condition(unsigned r)
  {
    ...
    r = r / 4;
bool mu__boolexpr20;
  if (!(!(mu_gBuf.mu_src.isundefined()))) mu__boolexpr20 = FALSE ;
  else {
  mu__boolexpr20 = (!(mu_ackBus[mu_n])) ; 
}
  ...
  void NextRule(unsigned & what_rule)
  {
	...
	if ( ( TRUE  ) ) {
bool mu__boolexpr21;
  if (!(!(mu_gBuf.mu_src.isundefined()))) mu__boolexpr21 = FALSE ;
  else {
...
bool mu__boolexpr22;
bool mu__boolexpr23;
  if (!((mu_p.mu_state) == (mu_P_V))) mu__boolexpr23 = FALSE ;
  else {
...
/********************
  Invariant records
 ********************/
int mu__invariant_36() // Invariant "P_I_ci_load_rset_0"
{
bool mu__boolexpr37;
bool mu__boolexpr38;
  if (!(mu_tracked)) mu__boolexpr38 = FALSE ;
  else {
  mu__boolexpr38 = (!(mu_start)) ; 
}
  ...
  for (mu_i = 0; ; mu_i=mu_i+1)
    {
      if (mu_reached_set.valid[(int)mu_i].value())
	{
	  if ( (mu_reached_set[mu_i]) == (mu_P_I) ) mu__intexpr42++;
	}
      if (mu_i == 3-1) break;
    }
  }
/*** end multisetcount 1 declaration ***/
  if (!((mu__intexpr42) == (0))) mu__boolexpr41 = FALSE ;
  else {
  ...
  for (mu_i = 0; ; mu_i=mu_i+1)
    {
      if (mu_reached_set.valid[(int)mu_i].value())
	{
	  if ( (mu_reached_set[mu_i]) == (mu_P_IVD) ) mu__intexpr43++;
	}
      if (mu_i == 3-1) break;
    }
  }
/*** end multisetcount 2 declaration ***/
  mu__boolexpr41 = ((mu__intexpr43) == (1)) ; 
}
  ...
  for (mu_i = 0; ; mu_i=mu_i+1)
    {
      if (mu_reached_set.valid[(int)mu_i].value())
	{
	  if ( (mu_reached_set[mu_i]) == (mu_P_V) ) mu__intexpr44++;
	}
      if (mu_i == 3-1) break;
    }
  }
/*** end multisetcount 3 declaration ***/
  mu__boolexpr40 = ((mu__intexpr44) == (1)) ; 
}
  if (!(mu__boolexpr40)) mu__boolexpr39 = FALSE ;
  else {
  mu__boolexpr39 = (mu_true) ; 
}
  mu__boolexpr37 = (!(mu__boolexpr39)) ; 
}
return mu__boolexpr37;
};

bool mu__condition_45() // Condition for Rule "P_I_ci_load_rset_0"
{
...
int mu__invariant_46() // Invariant "swmr"
{
bool mu__quant47; 
mu__quant47 = TRUE;
{
...
bool mu__boolexpr49;
bool mu__boolexpr50;
  if (!((mu_c1) != (mu_c2))) mu__boolexpr50 = FALSE ;
  else {
  mu__boolexpr50 = ((mu_Procs[mu_c1].mu_state) == (mu_P_V)) ; 
}
  if (!(mu__boolexpr50)) mu__boolexpr49 = TRUE ;
  else {
  mu__boolexpr49 = ((mu_Procs[mu_c2].mu_state) != (mu_P_V)) ; 
}
if ( !(mu__boolexpr49) )
...
bool mu__condition_51() // Condition for Rule "swmr"
{
...
int mu__invariant_52() // Invariant "values in valid state match last write"
{
bool mu__quant53; 
mu__quant53 = TRUE;
{
for(int mu_n = 1; mu_n <= 3; mu_n++) {
bool mu__boolexpr54;
  if (!((mu_Procs[mu_n].mu_state) == (mu_P_V))) mu__boolexpr54 = TRUE ;
  else {
...
bool mu__condition_55() // Condition for Rule "values in valid state match last write"
{
...
int mu__invariant_56() // Invariant "value in memory matches value of last write, when invalid"
{
bool mu__boolexpr57;
  if (!((mu_HomeNode.mu_state) == (mu_H_I))) mu__boolexpr57 = TRUE ;
  else {
  mu__boolexpr57 = ((mu_MainMemory) == (mu_LastWrite)) ; 
}
return mu__boolexpr57;
};

bool mu__condition_58() // Condition for Rule "value in memory matches value of last write, when invalid"
{
...
int mu__invariant_59() // Invariant "Invalid implies empty owner"
{
bool mu__boolexpr60;
  if (!((mu_HomeNode.mu_state) == (mu_H_I))) mu__boolexpr60 = TRUE ;
  else {
bool mu__quant61; 
mu__quant61 = TRUE;
{
...
{"Invalid implies empty owner", &mu__condition_62, NULL, },
{"value in memory matches value of last write, when invalid", &mu__condition_58, NULL, },
{"values in valid state match last write", &mu__condition_55, NULL, },
{"swmr", &mu__condition_51, NULL, },
{"P_I_ci_load_rset_0", &mu__condition_45, NULL, },
};
 ...
 ********************/
/*
just_ret:NoScalarset
prevProcs:NoScalarset
tracked:NoScalarset
start:NoScalarset
state_ret_val:NoScalarset
  ...
  if (args->symmetry_reduction.value)
    {
      if (  args->sym_alg.mode == argsym_alg::Exhaustive_Fast_Canonicalize
          ...
          if (Perm.In(i))
            {
              ...
              if (args->multiset_reduction.value)
                mu_start.MultisetSort();
              mu_state_ret_val.Permute(Perm,i);
              if (args->multiset_reduction.value)
                mu_state_ret_val.MultisetSort();
        ...
        if (ns != workingstate)
          ...
          if (args->multiset_reduction.value)
            mu_start.MultisetSort();
          mu_state_ret_val.Permute(Perm,0);
          if (args->multiset_reduction.value)
            mu_state_ret_val.MultisetSort();
