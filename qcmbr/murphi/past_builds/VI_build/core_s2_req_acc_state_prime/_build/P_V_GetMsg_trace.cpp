/******************************
  Program "build/core_s2_req_acc_state_prime/out/P_V_GetMsg_trace.m" compiled by "Caching Murphi Release 5.4.9.1"
...
#define PROTOCOL_NAME "build/core_s2_req_acc_state_prime/out/P_V_GetMsg_trace"
...
/*** Variable declaration ***/
mu_1_MessageType mu_prevSentMsg("prevSentMsg",8);

/*** Variable declaration ***/
mu_1_Node mu_prevMsgDst("prevMsgDst",16);

/*** Variable declaration ***/
mu_1_Node mu_prevMsgSrc("prevMsgSrc",24);

/*** Variable declaration ***/
mu_1_MessageType mu_prevMsg("prevMsg",32);

/*** Variable declaration ***/
mu_1_ProcStateEnum mu_prevProcs("prevProcs",40);

/*** Variable declaration ***/
mu_1_Proc mu_selc("selc",48);

/*** Variable declaration ***/
mu_1__type_2 mu_prevSentMsgSet("prevSentMsgSet",56);

/*** Variable declaration ***/
mu_1_HomeState mu_HomeNode("HomeNode",80);

/*** Variable declaration ***/
mu_1__type_3 mu_Procs("Procs",96);

/*** Variable declaration ***/
mu_1_Value mu_LastWrite("LastWrite",144);

/*** Variable declaration ***/
mu_1_Value mu_MainMemory("MainMemory",152);

/*** Variable declaration ***/
mu_1_Message mu_gBuf("gBuf",160);

/*** Variable declaration ***/
mu_1__type_4 mu_ackBus("ackBus",192);

/*** Variable declaration ***/
mu_1__type_5 mu_msgBuf("msgBuf",224);

/*** Variable declaration ***/
mu_0_boolean mu_msgBufNonEmpty("msgBufNonEmpty",352);

/*** Variable declaration ***/
mu_1__type_6 mu_curRequsts("curRequsts",360);
...
if (mu_m.isundefined())
  ...
  mu_cur_node = mu_m;
mu_prevSentMsg.undefine();
mu_prevMsgDst.undefine();
mu_prevMsgSrc.undefine();
mu_prevMsg.undefine();
...
bool mu__boolexpr7;
  ...
  mu__boolexpr7 = ((mu_cur_node) == (mu_selc)) ; 
}
if ( mu__boolexpr7 )
{
...
bool mu__boolexpr8;
  ...
  mu__boolexpr8 = ((mu_cur_node) == (mu_selc)) ; 
}
if ( mu__boolexpr8 )
{
...
void world_class::print_statistic()
{
  ...
  mu_prevSentMsg.print_statistic();
  mu_prevMsgDst.print_statistic();
  mu_prevMsgSrc.print_statistic();
...
void world_class::print_diff( state *prevstate )
{
  if ( prevstate != NULL )
  {
    ...
    mu_prevSentMsg.print_diff(prevstate);
    mu_prevMsgDst.print_diff(prevstate);
    mu_prevMsgSrc.print_diff(prevstate);
...
void world_class::to_state(state *newstate)
{
  ...
  mu_prevSentMsg.to_state( newstate );
  mu_prevMsgDst.to_state( newstate );
  mu_prevMsgSrc.to_state( newstate );
...
/********************
  Invariant records
 ********************/
int mu__invariant_28() // Invariant "P_V_rec_GetMsg"
{
bool mu__boolexpr29;
bool mu__boolexpr30;
bool mu__boolexpr31;
bool mu__boolexpr32;
  if (!(!(mu_prevProcs.isundefined()))) mu__boolexpr32 = FALSE ;
  else {
  mu__boolexpr32 = ((mu_prevProcs) == (mu_P_V)) ; 
}
  if (!(mu__boolexpr32)) mu__boolexpr31 = FALSE ;
  else {
  mu__boolexpr31 = (!(mu_prevMsg.isundefined())) ; 
}
  if (!(mu__boolexpr31)) mu__boolexpr30 = FALSE ;
  else {
  mu__boolexpr30 = ((mu_prevMsg) == (mu_GetMsg)) ; 
}
...
bool mu__boolexpr34;
bool mu__boolexpr35;
  if (!(!(mu_prevProcs.isundefined()))) mu__boolexpr35 = FALSE ;
  else {
  mu__boolexpr35 = ((mu_prevProcs) != (mu_Procs[mu_selc].mu_state)) ; 
}
  if (mu__boolexpr35) mu__boolexpr34 = TRUE ;
  else {
bool mu__boolexpr36;
  if (!(!(mu_prevMsgDst.isundefined()))) mu__boolexpr36 = FALSE ;
  else {
  mu__boolexpr36 = ((mu_prevMsgDst) == (mu_selc)) ; 
}
  mu__boolexpr34 = (mu__boolexpr36) ; 
}
  if (mu__boolexpr34) mu__boolexpr33 = TRUE ;
  else {
  mu__boolexpr33 = (!(mu_prevSentMsg.isundefined())) ; 
}
...
bool mu__condition_37() // Condition for Rule "P_V_rec_GetMsg"
{
...
{"P_V_rec_GetMsg", &mu__condition_37, NULL, },
};
  ...
  if (args->symmetry_reduction.value)
    {
      if (  args->sym_alg.mode == argsym_alg::Exhaustive_Fast_Canonicalize
          ...
          if (Perm.In(i))
            {
              ...
              if (args->multiset_reduction.value)
                mu_selc.MultisetSort();
              mu_prevMsgDst.Permute(Perm,i);
              if (args->multiset_reduction.value)
                mu_prevMsgDst.MultisetSort();
              mu_cur_node.Permute(Perm,i);
              if (args->multiset_reduction.value)
                mu_cur_node.MultisetSort();
              mu_prevMsgSrc.Permute(Perm,i);
              if (args->multiset_reduction.value)
                mu_prevMsgSrc.MultisetSort();
        ...
        if (ns != workingstate)
          ...
          if (args->multiset_reduction.value)
            mu_selc.MultisetSort();
          mu_prevMsgDst.Permute(Perm,0);
          if (args->multiset_reduction.value)
            mu_prevMsgDst.MultisetSort();
          mu_cur_node.Permute(Perm,0);
          if (args->multiset_reduction.value)
            mu_cur_node.MultisetSort();
          mu_prevMsgSrc.Permute(Perm,0);
          if (args->multiset_reduction.value)
            mu_prevMsgSrc.MultisetSort();
...
void SymmetryClass::Heuristic_Fast_Canonicalize(state* s)
{
  ...
  mu_prevMsgDst.SimpleCanonicalize(Perm);

  mu_cur_node.SimpleCanonicalize(Perm);

  mu_prevMsgSrc.SimpleCanonicalize(Perm);
