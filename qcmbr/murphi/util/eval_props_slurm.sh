...
usage() {
  echo "Usage: $0 -s <stage> [-d <build_dir>] [-n] [-P] [-p <partition>] [-t <time>] [-m <mem>] [-c <cpus>] [-J <job_name>]"
}

while getopts "d:s:np:Pt:m:c:J:" opt; do
  case $opt in
    d) tar_build="$OPTARG";;
...
if [ "$no_overwrite" -eq 1 ]; then
  ...
  for t in $TESTS; do
    if [ -f "${OUTPUTDIR}/${t}.txt" ]; then
      if grep -q "terminated" "${OUTPUTDIR}/${t}.txt"; then 
        echo "$t" >> "$TEST_LIST_FILE"
      else
