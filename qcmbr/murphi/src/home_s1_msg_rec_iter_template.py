...
base_ff_name = f"{hstate}_{hstate_prime}_{prefix}"
resff = f"{tardir}/{trace_stepname}/_build/{hstate}_{hstate_prime}_{prefix}_trace.txt"
assert_transition_template = '''
invariant "{hstate}_to_{hstate_prime}"
...
def build_msg_set_clause(msg_set):
  ...
  if msg_type_arr is None:
    ...
    for group_name, group_set in msg_set.items():
      for m_, v in group_set.items():
        msg_set_ss.append(f"msg_imm_{prefix}_set_{group_name}[{m_}] = {v}")
  if len(msg_set_ss) == 0:
    return "false"
...
def write_iter_file(iter_count, acc_):
  ...
  with open(outff, "w") as outff_h:
    with open(f"{tardir}/{stepname}/out/{hstate}_{hstate_prime}_{prefix}_baseff.m", "r") as f:
      for ln in f:
        outff_h.write(ln)
    outff_h.write(
      assert_transition_template.format(
        hstate=hstate,
        hstate_prime=hstate_prime,
        home_state_expr=home_state_expr,
...
def main():
  ...
  while True:
    ...
    prop = f"{hstate}_to_{hstate_prime}"
